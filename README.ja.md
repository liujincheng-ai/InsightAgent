# <img src="./assets/LOGO_SMALL.png" alt="Logo" style="vertical-align: middle; height: 24px;" /> InsightAgent: データベースとの対話を革新するプライベートLLM技術
...
[**ドキュメント**](https://liujincheng-ai.github.io/InsightAgent/docs/overview/) | [**チームに連絡します**](https://github.com/liujincheng-ai/InsightAgent/blob/main/README.zh.md#%E8%81%94%E7%B3%BB%E6%88%91%E4%BB%AC) | [**コミュニティ**](https://github.com/liujincheng-ai/InsightAgent/discussions) | [**論文**](https://arxiv.org/pdf/2312.17449.pdf)
...
🤖 **InsightAgentは、AWEL（エージェントワークフロー式言語）とエージェントを備えたオープンソースのAIネイティブデータアプリ開発フレームワークです。**
...
### 紹介
InsightAgentのアーキテクチャは以下の図に示されています：
...
- **RAG（Retrieval Augmented Generation）**：現在、RAGは最も実用的に実装され、緊急に必要とされる領域です。InsightAgentは、RAGの機能を使用して知識ベースのアプリケーションを構築できるようにする、RAGに基づくフレームワークをすでに実装しています。

- **GBI（Generative Business Intelligence）**：Generative BIはInsightAgentプロジェクトのコア機能の1つであり、企業のレポート分析とビジネスインサイトを構築するための基本的なデータインテリジェンス技術を提供します。

- **ファインチューニングフレームワーク**：モデルのファインチューニングは、任意の企業が垂直およびニッチなドメインで実装するために不可欠な機能です。InsightAgentは、InsightAgentプロジェクトとシームレスに統合される完全なファインチューニングフレームワークを提供します。最近のファインチューニングの取り組みでは、Spiderデータセットに基づいて82.5%の実行精度を達成しています。

- **データ駆動型マルチエージェントフレームワーク**：InsightAgentは、データに基づいて継続的に意思決定を行い、実行するためのデータ駆動型自己進化型マルチエージェントフレームワークを提供します。

- **データファクトリー**：データファクトリーは、主に大規模モデルの時代における信頼できる知識とデータのクリーニングと処理に関するものです。

- **データソース**：InsightAgentのコア機能に生産ビジネスデータをシームレスに接続するために、さまざまなデータソースを統合します。

#### サブモジュール
- [DB-GPT-Hub](https://github.com/eosphoros-ai/DB-GPT-Hub) 大規模言語モデル（LLM）上での教師ありファインチューニング（SFT）を適用することにより、高性能なText-to-SQLワークフロー。

- [dbgpts](https://github.com/eosphoros-ai/dbgpts)  dbgptsは、InsightAgent上で構築されたいくつかのデータアプリ、AWELオペレータ、AWELワークフローテンプレート、およびエージェントを含む公式リポジトリです。
...
- [InsightAgent-Plugins](https://github.com/liujincheng-ai/InsightAgent-Plugins) Auto-GPTプラグインを直接実行できるInsightAgentプラグイン
- [GPT-Vis](https://github.com/eosphoros-ai/GPT-Vis) 可視化プロトコル


### AIネイティブデータアプリ
- 🔥🔥🔥 [V0.7.0 リリース | 重要なアップグレードのセット](https://liujincheng-ai.github.io/InsightAgent/blog/db-gpt-v070-release)
  - [サポート MCP Protocol](https://github.com/liujincheng-ai/InsightAgent/pull/2497)
...
[**使用チュートリアル**](https://liujincheng-ai.github.io/InsightAgent/docs/overview)
- [**インストール**](https://liujincheng-ai.github.io/InsightAgent/docs/installation)
  - [Docker](https://liujincheng-ai.github.io/InsightAgent/docs/installation/docker)
  - [ソースコード](https://liujincheng-ai.github.io/InsightAgent/docs/installation/sourcecode)
- [**クイックスタート**](https://liujincheng-ai.github.io/InsightAgent/docs/quickstart)
- [**アプリケーション**](https://liujincheng-ai.github.io/InsightAgent/docs/operation_manual)
  - [アプリの使用](https://liujincheng-ai.github.io/InsightAgent/docs/application/app_usage)
  - [AWELフローの使用](https://liujincheng-ai.github.io/InsightAgent/docs/application/awel_flow_usage)
- [**デバッグ**](https://liujincheng-ai.github.io/InsightAgent/docs/operation_manual/advanced_tutorial/debugging)
- [**高度な使用法**](https://liujincheng-ai.github.io/InsightAgent/docs/application/advanced_tutorial/cli)
  - [SMMF](https://liujincheng-ai.github.io/InsightAgent/docs/application/advanced_tutorial/smmf)
  - [ファインチューニング](https://liujincheng-ai.github.io/InsightAgent/docs/application/fine_tuning_manual/dbgpt_hub)
...
- **プライベートドメインQ&A＆データ処理**
  ...
  InsightAgentプロジェクトは、知識ベースの構築を改善し、構造化および非構造化データの両方の効率的なストレージと検索を可能にする一連の機能を提供します。これらの機能には、複数のファイル形式のアップロードのサポート、カスタムデータ抽出プラグインの統合、および大量の情報を効果的に管理するための統一されたベクトルストレージと検索機能が含まれます。

- **マルチデータソース＆GBI（Generative Business Intelligence）**

  InsightAgentプロジェクトは、Excel、データベース、データウェアハウスなどのさまざまなデータソースとの自然言語のシームレスな対話を容易にします。これらのソースから情報を照会および取得するプロセスを簡素化し、直感的な会話を行い、洞察を得ることができます。さらに、InsightAgentは分析レポートの生成をサポートし、ユーザーに貴重なデータの要約と解釈を提供します。
...
<table>
  ...
  - [サポートされているLLMの詳細](https://liujincheng-ai.github.io/InsightAgent/docs/modules/smmf)
...
- サポートされているデータソース
  - [データソース](https://liujincheng-ai.github.io/InsightAgent/docs/modules/connections)

## 画像
🌐 [AutoDLイメージ](https://www.codewithgpu.com/i/liujincheng-ai/InsightAgent/dbgpt)

## 貢献

- 新しい貢献のための詳細なガイドラインを確認するには、[貢献方法](https://github.com/liujincheng-ai/InsightAgent/blob/main/CONTRIBUTING.md)を参照してください。
...
## 引用
もし`InsightAgent`があなたの研究や開発に役立つと感じた場合、以下の論文を引用してください。

InsightAgentの全体的なアーキテクチャについて知りたい場合は、<a href="https://arxiv.org/abs/2312.17449" target="_blank">論文</a>と<a href="https://arxiv.org/abs/2404.10209" target="_blank">論文</a>を引用してください。

InsightAgentを使用してAgent開発に関する内容について知りたい場合は、<a href="https://arxiv.org/abs/2412.13520" target="_blank">論文</a>を引用してください。
```bibtex
...
[![Star History Chart](https://api.star-history.com/svg?repos=liujincheng-ai/InsightAgent&type=Date)](https://star-history.com/#liujincheng-ai/InsightAgent)
