...
To store the knowledge in a vector store, we need an embedding model, InsightAgent supports
...
chunks = asyncio.run(assembler_task.call("https://liujincheng-ai.github.io/InsightAgent/docs/awel/"))
...
To build a RAG program, we need a LLM, here are some of the LLMs that InsightAgent supports:
