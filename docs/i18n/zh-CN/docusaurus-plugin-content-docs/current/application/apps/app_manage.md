...
As shown in the figure, the application management interface. First, let's take a look at the creation of an application. In InsightAgent, four application creation modes are provided.
...
Next, we will explain the creation of applications in each mode respectively. Native application mode In the early versions of InsightAgent, six types of native application scenarios were provided, such as `Chat DB`, `Chat Data`, `Chat Dashboard`, `Chat Knowledge Base`, `Chat Normal`, `Chat Excel`, etc.
