...
## Observability In InsightAgent

InsightAgent provides observability through the following mechanisms:
- **Logging**: InsightAgent logs various events and metrics to help you understand the internal state of the system.
- **Tracing**: InsightAgent provides tracing capabilities to help you understand the flow of requests through the system.

## Logging

You can configure the logging level and storage location for InsightAgent logs. By default,
logs are stored in the `logs` directory in the InsightAgent root directory. You can change
the log level and storage location by setting the `DBGPT_LOG_LEVEL` and `DBGPT_LOG_DIR` environment.


## Tracing

InsightAgent has built-in tracing capabilities that allow you to trace the flow of requests
through the system.
...
InsightAgent will store traces in the `traces` directory in the InsightAgent logs directory, by default,
...
InsightAgent also supports [OpenTelemetry](https://opentelemetry.io/) for distributed tracing.
...
```bash
## Whether to enable InsightAgent send trace to OpenTelemetry
...
Here, we use Jaeger as an example to show how to use OpenTelemetry to trace InsightAgent.

### Jaeger Support

Here is an example of how to use Jaeger to trace InsightAgent with docker:
...
If you want to use docker-compose to start InsightAgent and Jaeger, you can use the following
...
In the above screenshot, you can see the trace of cross-service communication between the InsightAgent controller, LLM worker, and webserver.
