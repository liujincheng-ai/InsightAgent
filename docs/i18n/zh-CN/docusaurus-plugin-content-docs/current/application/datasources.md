...
InsightAgent 的数据源模块用于管理企业中的结构化和半结构化数据资产，把数据库、数仓、数据湖等系统接入到 InsightAgent 框架中，从而快速构建面向数据场景的智能应用和大模型能力。目前 InsightAgent 已支持多种常见数据源，也支持自定义扩展。
...
当前 InsightAgent 文档已覆盖或正在补充以下数据源集成：
...
## 数据源在 InsightAgent 中的作用

接入数据源之后，InsightAgent 可以基于这些结构化数据能力支持：
