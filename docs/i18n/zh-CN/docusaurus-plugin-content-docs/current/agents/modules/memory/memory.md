...
In InsightAgent agents, there are three main memory operations:
...
In InsightAgent agents, there are four main memory structures:
...
In the above example, we override the `read_memories` to read the memories from the memory, in InsightAgent,
...
In this document, we introduced the memory module in InsightAgent agents, and how to use the memory in agents.
In following sections, we will introduce how to use each memory structure in InsightAgent agents.