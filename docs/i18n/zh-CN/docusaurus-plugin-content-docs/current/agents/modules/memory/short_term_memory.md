...
Like human short-term memory, the short-term memory in InsightAgent agents can be enhanced by outside observations.
...
If you have deployed [InsightAgent cluster](../../../installation/model_service/cluster) and
...
agent_memory: AgentMemory = AgentMemory(memory=memory)
```
In InsightAgent, the core interface is asynchronous and non-blocking, so we use `ThreadPoolExecutor` to
