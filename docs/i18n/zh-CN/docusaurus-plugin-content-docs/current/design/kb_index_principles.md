...
> 讲解 InsightAgent 知识库如何对上传文档做索引——从原始文件到可被检索的索引。
...
> 一句话:**向量、关键词、知识图谱**是三种持久化索引方法。**结构索引**和**代码图谱**是 InsightAgent 在它们之上
> 构建的两种额外索引形态。
...
### 为什么单列(尽管它没有自己的索引方法)
索引阶段 InsightAgent **持久化**的是每个 chunk 上的标题路径,**并不**持久化这棵树。**检索时**检索器才根据
