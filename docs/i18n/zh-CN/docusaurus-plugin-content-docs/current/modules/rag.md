...
InsightAgent 实现的 **多源 RAG(MS-RAG)** 框架远超基础文档问答:它支持多种知识源(文档、URL、数据库、知识图谱、git 仓库)、多种索引策略,并与 InsightAgent 的 agent 和工作流生态深度集成。知识库对话由 **agentic RAG** 循环完成——agent 可以改写问题、多次检索、融合并重排结果、产出带引用的回答——而不是单次"检索-生成"。
...
InsightAgent 通过 `index_methods`(字符串列表)按知识空间选择要建哪些索引。三种索引*方法*是持久化的;**结构索引**和**代码图谱**是叠加在它们之上的两种形态。所有索引都作用在*同一批 chunk* 上,所以切分质量决定检索质量。
...
用户在知识库上提问时,InsightAgent **不是**单次"检索-生成",而是由 **agent** 驱动循环:
...
正是这个 agentic 循环——多步检索、问题改写、结果融合重排、引用——让 InsightAgent 能回答单次 RAG 应付不了的复杂或多部分问题。完整流程见 [Agentic RAG 对话原理](/docs/design/agentic_rag_principles)。
...
InsightAgent 支持从多种类型的源加载知识。Web UI 上传时可选数据源类型:
...
启用 **KnowledgeGraph** 索引方法时,InsightAgent 构建的是**一组图**,而非单张图。它们共用一条构建链路,都支持沿边检索:
...
切分是 RAG 质量的关键——它是所有索引的共同地基。InsightAgent 支持多种切分策略:
...
InsightAgent 支持多种把文本转向量的 embedding 模型:
...
可配置检索策略。InsightAgent 支持 Semantic / Keyword / Hybrid / Tree 等多种模式,按场景在知识库设置里选择。
...
| 源代码 | [GitHub](https://github.com/liujincheng-ai/InsightAgent/tree/main/packages/dbgpt-core/src/dbgpt/rag) |