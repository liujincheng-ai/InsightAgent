# 🚀 InsightAgent V0.8.2 — 更完整的 Agentic 数据工作流：多文件分析、知识检索与并行子 Agent
...
当包含附件的对话被保存为定时任务时，InsightAgent 会冻结一份任务级文件副本。每次定时运行都基于同一份文件快照重放，不依赖可能已变化的会话上传文件。
...
- 新增会话级多文件上传、预览与 Agentic 分析（[#3206](https://github.com/liujincheng-ai/InsightAgent/pull/3206)）
- 新增 Agentic 知识库检索，支持索引方式配置与迭代 RAG（[#3160](https://github.com/liujincheng-ai/InsightAgent/pull/3160)）
- 新增并行子 Agent 委派与执行（[#3161](https://github.com/liujincheng-ai/InsightAgent/pull/3161)）
- 为 Agent 内置工具增加人在回路的交互式追问（[#3107](https://github.com/liujincheng-ai/InsightAgent/pull/3107)）
- 新增 OrcaRouter 代理 Provider，通过 OpenAI 兼容接口接入 OrcaRouter 模型目录（[#3186](https://github.com/liujincheng-ai/InsightAgent/pull/3186)）
- 新增基于 PyODPS SQLAlchemy Dialect 的阿里云 MaxCompute（ODPS）数据源，并支持从 Web UI 配置（修复 [#3105](https://github.com/liujincheng-ai/InsightAgent/issues/3105)）（[#3178](https://github.com/liujincheng-ai/InsightAgent/pull/3178)）

## 问题修复

- 修复 Chart 或 SQL 运行缺少 `db_name` 时触发 `KeyError` 的问题（[#3199](https://github.com/liujincheng-ai/InsightAgent/pull/3199)）
- 在 `update_flow` 的 PUT 路径中加入 Flow UID（修复 [#3193](https://github.com/liujincheng-ai/InsightAgent/issues/3193)）（[#3196](https://github.com/liujincheng-ai/InsightAgent/pull/3196)）
- MySQL ORM 为较大的 Agent 消息和执行报告使用 `LONGTEXT`（[#3189](https://github.com/liujincheng-ai/InsightAgent/pull/3189)）
- 将 `EXAMPLE_1` 数据库的创建移动到 Schema 文件末尾（[#3183](https://github.com/liujincheng-ai/InsightAgent/pull/3183)）
- 使用结构化最终答案协议，将引用信息与最终答案分离（[#3182](https://github.com/liujincheng-ai/InsightAgent/pull/3182)）
- 校验文件上传接口的用户身份，并约束解析后的路径（修复 [#3104](https://github.com/liujincheng-ai/InsightAgent/issues/3104)）（[#3184](https://github.com/liujincheng-ai/InsightAgent/pull/3184)）
- 仅将文件名严格等于 `SKILL.md` 的文件加载为 Markdown Skill（[#3175](https://github.com/liujincheng-ai/InsightAgent/pull/3175)）
- 移除 macOS TTS 的 Shell 解释，防止命令注入（修复 [#3129](https://github.com/liujincheng-ai/InsightAgent/issues/3129)）（[#3174](https://github.com/liujincheng-ai/InsightAgent/pull/3174)）
- 兼容 LLM 输出中包含多个 JSON 代码块的情况（[#3117](https://github.com/liujincheng-ai/InsightAgent/pull/3117)）
- 在沙箱环境中渲染 Agent Jinja2 模板（[#3111](https://github.com/liujincheng-ai/InsightAgent/pull/3111)）
- 避免 `inner_copy_and_install` 将构建失败报告为成功（[#3141](https://github.com/liujincheng-ai/InsightAgent/pull/3141)）
- 在存在拆分 Chunk 时正确反序列化表格 Chunk（[#3140](https://github.com/liujincheng-ai/InsightAgent/pull/3140)）
- 客户端的 `create_datasource` 和 `create_flow` 调用改用 POST，而不是 GET（[#3138](https://github.com/liujincheng-ai/InsightAgent/pull/3138)）
- 支持配置 CORS 允许来源，并修正通配符与凭据的组合处理（[#3123](https://github.com/liujincheng-ai/InsightAgent/pull/3123)）
- 修复 `StreamedBytesIO` 忽略负数读取长度、丢失 `SEEK_END` 位置的问题（[#3136](https://github.com/liujincheng-ai/InsightAgent/pull/3136)）
- 在 `VariablesProvider._convert_to_value_type` 中正确处理原生 `bool` 值（[#3135](https://github.com/liujincheng-ai/InsightAgent/pull/3135)）
- 在简化的 fsspec 路径中保留 `bucket` 名称（[#3134](https://github.com/liujincheng-ai/InsightAgent/pull/3134)）
- 在 `ExcelKnowledge._load` 中处理无表头工作表和包含多个工作表的工作簿（[#3137](https://github.com/liujincheng-ai/InsightAgent/pull/3137)）
- 修正 `TeiRerankEmbeddings._parse_results` 对空值和非列表结果的判断（[#3133](https://github.com/liujincheng-ai/InsightAgent/pull/3133)）
- 避免前端 `handleChat` 的暂时性死区问题（[#3132](https://github.com/liujincheng-ai/InsightAgent/pull/3132)）
- 允许在 TOML 配置中使用 Milvus 向量存储类型（[#3127](https://github.com/liujincheng-ai/InsightAgent/pull/3127)）
...
使用原有启动方式重启 InsightAgent 服务。启动后建议验证：
...
- [InsightAgent V0.8.1 发版说明](https://liujincheng-ai.github.io/InsightAgent/docs/next/changelog/Released_V0.8.1)
...
- [RAG 基础概念](https://liujincheng-ai.github.io/InsightAgent/docs/next/getting-started/concepts/rag)
- [知识库索引设计原则](https://liujincheng-ai.github.io/InsightAgent/docs/next/design/kb_index_principles)
