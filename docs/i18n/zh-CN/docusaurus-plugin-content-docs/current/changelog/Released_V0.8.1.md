# 🚀 InsightAgent V0.8.1 — 让 AI 数据助理走向生产：定时、连接与长程 Agent

V0.8.1 延续 V0.8.0 AI 数据助理的方向，把“一次性完成分析”的能力进一步沉淀为**可调度、可连接、可观察、可运维**的生产化工作流。

## 简介

InsightAgent V0.8.0 完成了从“对话问答”到“任务交付”的范式跃迁。V0.8.1 则面向真实团队的持续使用场景，回答几个更偏生产化的问题：
...
你只需先完成一次数据分析，再将该对话保存为任务。InsightAgent 会按 Cron 计划重放完整的 ReAct Agent 流程，每次运行都会生成新的对话与报告，并保留完整执行历史，方便审计、复盘和团队共享。
...
V0.8.1 通过 **MCP 连接器（MCP Connectors）** 将 InsightAgent Agent 的能力从数据库和本地 Skill 扩展到外部服务。Agent 现在可以通过 Model Context Protocol 接入外部工具，同时由用户掌控每次会话到底挂载哪些连接器。
...
| 凭据加密存储 | 连接器凭据加密保存在 InsightAgent 元数据库中，并在服务重启后自动恢复 |
...
这些改进让 InsightAgent 更适合承接需要多步推理、反复尝试和中间产物管理的复杂数据分析工作流。

### 🧱 模型、数据源与存储生态扩展

V0.8.1 进一步扩展了 InsightAgent 周边生态，让团队可以更方便地复用已有模型、数据库、向量存储和缓存基础设施。

#### LiteLLM 嵌入式 AI Gateway

InsightAgent 新增 **LiteLLM** 作为嵌入式代理 Provider，注册名为 `proxy/litellm`。它不是一个额外的代理服务，而是 InsightAgent 进程内直接调用 LiteLLM，让你通过统一入口访问 OpenAI、Anthropic、Vertex AI、Bedrock、Azure、Cohere、Mistral、Groq、Ollama 等 LiteLLM 支持的众多后端。
...
- **Valkey 向量客户端 `CLIENT SETNAME`**：使 InsightAgent 的连接在 Valkey 监控工具中可被清晰识别。
...
- 支持定时任务与 MCP 连接器（[#3095](https://github.com/liujincheng-ai/InsightAgent/pull/3095)）
- 新增上下文管理、任务计划追踪及对应前端 UI（[#3053](https://github.com/liujincheng-ai/InsightAgent/pull/3053)）
- 新增 LiteLLM 嵌入式 AI Gateway Provider（[#3043](https://github.com/liujincheng-ai/InsightAgent/pull/3043)）
- 新增 Qdrant 向量检索支持（[#3034](https://github.com/liujincheng-ai/InsightAgent/pull/3034)）
- 新增 Valkey 向量存储集成（[#3051](https://github.com/liujincheng-ai/InsightAgent/pull/3051)）
- 新增 Valkey 缓存存储集成（[#3057](https://github.com/liujincheng-ai/InsightAgent/pull/3057)）
- 为 Valkey 向量客户端增加 `CLIENT SETNAME`（[#3090](https://github.com/liujincheng-ai/InsightAgent/pull/3090)）
- 支持配置向量检索距离度量（[#3044](https://github.com/liujincheng-ai/InsightAgent/pull/3044)）
- 为 `ConnectorManager.get_connector` 增加 TTL 缓存（热缓存约 63s → 10ms 以内）（[#3046](https://github.com/liujincheng-ai/InsightAgent/pull/3046)）
- 新增 openGauss 数据源支持（[#3007](https://github.com/liujincheng-ai/InsightAgent/pull/3007)）
- 为 StarRocks 实现 `VARBINARY` 和 `BINARY` 类型（[#3062](https://github.com/liujincheng-ai/InsightAgent/pull/3062)）
- MiniMax 默认模型升级到 M3（[#3093](https://github.com/liujincheng-ai/InsightAgent/pull/3093)）
- 加强 `~/.dbgpt/configs/<profile>.toml` 文件权限为 `0o600`（[#3077](https://github.com/liujincheng-ai/InsightAgent/pull/3077)）

## 问题修复

- 修复 Web UI 创建 DuckDB 数据源的问题（[#3009](https://github.com/liujincheng-ai/InsightAgent/pull/3009)）
- 支持 DeepSeek V4 Pro（[#3079](https://github.com/liujincheng-ai/InsightAgent/pull/3079)）
- Markdown 知识库默认使用 size chunking（修复 [#3030](https://github.com/liujincheng-ai/InsightAgent/issues/3030)）（[#3033](https://github.com/liujincheng-ai/InsightAgent/pull/3033)）
- 限制个人 Skill 脚本执行（[#3071](https://github.com/liujincheng-ai/InsightAgent/pull/3071)）
- 校验示例文件名（[#3066](https://github.com/liujincheng-ai/InsightAgent/pull/3066)）
- 校验 Skill 上传文件名（[#3065](https://github.com/liujincheng-ai/InsightAgent/pull/3065)）
- 约束 Python 上传文件名（[#3064](https://github.com/liujincheng-ai/InsightAgent/pull/3064)）
- 处理知识空间 ID 响应（[#3070](https://github.com/liujincheng-ai/InsightAgent/pull/3070)）
- 兼容多步 ReAct 输出（[#3074](https://github.com/liujincheng-ai/InsightAgent/pull/3074)）
- 告知 LLM 当前表列表是 TOP-K 子集，而非全库表列表（[#3045](https://github.com/liujincheng-ai/InsightAgent/pull/3045)）
- 扩展 `gpts_messages.content` 字段以容纳更长的 Agent 消息（[#3055](https://github.com/liujincheng-ai/InsightAgent/pull/3055)）
- 兼容 Milvus 2.5+（[#3042](https://github.com/liujincheng-ai/InsightAgent/pull/3042)）
- 增加按 Chunk 粒度容错和按数据库粒度索引锁（[#3040](https://github.com/liujincheng-ai/InsightAgent/pull/3040)）
- 为 MSSQL `get_fields()` 实现 SQL Server 兼容的 `INFORMATION_SCHEMA` 查询（[#3039](https://github.com/liujincheng-ai/InsightAgent/pull/3039)）
- 为知识库模块接口补充认证依赖（[#3038](https://github.com/liujincheng-ai/InsightAgent/pull/3038)）
- 修复 BranchOperator 错误跳过共享下游节点的问题（修复 [#2935](https://github.com/liujincheng-ai/InsightAgent/issues/2935)）（[#3035](https://github.com/liujincheng-ai/InsightAgent/pull/3035)）
- 遵循已配置的通义 Embedding 模型名称（修复 [#3029](https://github.com/liujincheng-ai/InsightAgent/issues/3029)）（[#3032](https://github.com/liujincheng-ai/InsightAgent/pull/3032)）
- 写入 Code Interpreter 临时脚本时显式使用 UTF-8 编码（[#3023](https://github.com/liujincheng-ai/InsightAgent/pull/3023)）
...
使用你原来的启动方式重启 InsightAgent 服务。启动后建议验证：
...
- [InsightAgent V0.8.0 发版说明](https://liujincheng-ai.github.io/InsightAgent/docs/next/changelog/Released_V0.8.0)
- [快速开始](https://liujincheng-ai.github.io/InsightAgent/docs/overview/)
- [安装指南](https://liujincheng-ai.github.io/InsightAgent/docs/next/installation/)
- [定时任务](https://liujincheng-ai.github.io/InsightAgent/docs/next/application/scheduled_tasks)
- [MCP 连接器](https://liujincheng-ai.github.io/InsightAgent/docs/next/application/mcp_connectors)
