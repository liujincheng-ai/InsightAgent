...
In this document, we will guide you through the process of developing a data analysis app using InsightAgent.
...
These three capabilities can be utilized within a single conversation based on the intent recognition ability provided by InsightAgent. The data assistant will match appropriate sub-agent applications to answer questions in corresponding domains based on the user's inquiries.
...
We use the agent module provided by InsightAgent to build a RAG-based question-answering assistant. InsightAgent has some built-in agents, such as
...
:::tip
Note:  The agent application shown in this tutorial is built based on the Summarizer agent. The Summarizer agent is a built-in agent of InsightAgent. See the [source code](https://github.com/liujincheng-ai/InsightAgent/blob/main/dbgpt/agent/expand/summary_assistant_agent.py) for the relevant code implementation. In actual use, the relevant code can be further modified according to specific scenarios. Customization and optimization. Or customize the agent based on this case
:::
...
For data preparation, please refer to the [data preparation](https://github.com/liujincheng-ai/InsightAgent/blob/main/docker/examples/dashboard/test_case_mysql_data.py) section in the document.
