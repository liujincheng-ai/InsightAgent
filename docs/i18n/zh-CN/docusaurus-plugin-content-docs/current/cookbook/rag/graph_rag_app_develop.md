...
In this example, we will show how to use the Graph RAG framework in InsightAgent. Using a graph database to implement RAG can, to some extent, alleviate the uncertainty and interpretability issues brought about by vector database retrieval.

You can refer to the python example file `InsightAgent/examples/rag/graph_rag_example.py` in the source code. This example demonstrates how to load knowledge from a document and store it in a graph store. Subsequently, it recalls knowledge relevant to your question by searching for triplets in the graph store.
...
To store the knowledge in graph, we need an graph database, [TuGraph](https://github.com/TuGraph-family/tugraph-db) is the first graph database supported by InsightAgent.
...
To build a Graph RAG program, we need a LLM, here are some of the LLMs that InsightAgent supports:
...
If you have deployed [InsightAgent cluster](/docs/installation/model_service/cluster) and
...
Set variables below in `.env` file, let InsightAgent know how to connect to TuGraph.
...
When using a graph database as the underlying knowledge storage platform, it is necessary to build a knowledge graph to facilitate the archiving and retrieval of documents. InsightAgent leverages the capabilities of large language models to implement an integrated knowledge graph, while still maintaining the flexibility to freely connect to other knowledge graph systems and graph database systems.
...
async def test_community_graph_rag():
    await __run_graph_rag(
        ...
        question="What's the relationship between TuGraph and InsightAgent ?",
    )
...
Then, upload the documents ([graphrag-test.md](https://github.com/liujincheng-ai/InsightAgent/blob/main/examples/test_files/graphrag-test.md)) and process them automatically (markdown header by default).
...
|                     | InsightAgent                | GraphRAG(microsoft)  |
| ------------------- | --------------------- | -------------------- |
...
· InsightAgent: InsightAgent是一个私有化的LLM技术数据库交互方式，支持多种高级功能，旨在推动数据3.0时代的到来。它提供了多模型管理、信息检索优化和智能体协作等功能。
...
整体而言，知识库展示了图数据库及其相关技术的广泛应用和快速进步，强调了InsightAgent和TuGraph等工具在数据智能和处理中的重要角色。
```
...
```
InsightAgent社区和TuGraph社区在社区贡献，社区生态，开发者的这几个方面的联系和区别分别是什么？
```
...
```
· 社区贡献:
  · InsightAgent社区: 由多个开发者和公司支持，开发者积极参与代码提交、审查和问题反馈，展现出强大的社区协作能力。主要合作项目包括Langchain-Chatchat、LLaMA-Factory、FastChat等。
  · TuGraph社区: 吸引了众多开发者参与，专注于提升TuGraph DB的功能和性能，与多个开源工具共同开发，形成了活跃的贡献模式。
· 社区生态:
  · InsightAgent社区: 主要围绕私有化LLM技术数据库交互方式，支持多种高级功能，促进数据智能应用开发，构建了一个以AI和数据处理为核心的生态。
  · TuGraph社区: 依托于图数据库领域，尤其是TuGraph DB和TuGraph Analytics，形成了以大数据处理和实时计算为核心的生态，涉及多个行业应用，如金融风控和社交网络。
· 开发者:
  · InsightAgent社区: 包含多个GitHub用户，开发者主要集中在AI和数据处理领域，积极参与开源项目的开发和维护。
  · TuGraph社区: 涉及来自不同国家的开发者，特别是在图数据库和大数据分析领域的专业人员，强调多领域协作与贡献。
总体来看，InsightAgent社区和TuGraph社区在贡献方式和生态领域有所不同，前者更侧重于AI和数据交互技术，后者则专注于图数据库技术及其应用。
```

- **GraphRAG(microsoft) Answer**
```
InsightAgent社区与TuGraph社区的比较
  InsightAgent社区和TuGraph社区在多个方面展现了各自的特点和贡献，尤其是在社区贡献、生态系统和开发者参与等方面。以下是对这两个社区的联系与区别的详细分析。
社区贡献
  InsightAgent社区
    InsightAgent社区围绕InsightAgent框架展开，整合了多个组织和资源，致力于AI和数据驱动应用的开发。该社区的主要贡献者包括Hiyouga、LM-Sys和Langchain-AI等组织，这些组织通过合作推动AI模型和应用的发展。InsightAgent的开发者们积极参与知识共享和技术创新，推动了AI应用的多样化和实用性。
  TuGraph社区
...
社区生态
  InsightAgent社区
    InsightAgent社区的生态系统是一个多元化的合作网络，涵盖了多个组织和技术平台。该社区通过整合不同的技术和数据源，支持从聊天系统到企业报告等多种应用，展现出其在AI领域的广泛适用性。InsightAgent的生态系统强调了组织间的协作与知识共享，促进了技术的快速发展。
  TuGraph社区
...
开发者参与
  InsightAgent社区
    在InsightAgent社区中，开发者的参与主要体现在对AI应用的开发和优化上。社区内的开发者通过贡献代码、参与讨论和解决问题，推动了InsightAgent框架的不断完善。该社区的开发者们来自不同国家和地区，展现了全球范围内对AI技术的关注和参与。
  TuGraph社区
...
总结
  总体而言，InsightAgent社区和TuGraph社区在社区贡献、生态系统和开发者参与等方面各具特色。InsightAgent社区更侧重于AI应用的多样性和组织间的合作，而TuGraph社区则专注于图数据的高效管理和分析。两者的共同点在于都强调了开源和社区合作的重要性，推动了各自领域的技术进步和应用发展。
```

### Retrieval Of Document Structure

In version 0.6.1 of InsightAgent, we have added a new feature:
...
In the latest version of InsightAgent, we have implemented a new feature:
...
Set the variables below in the `.env` file to enable similarity search in InsightAgent.
...
Set the variables below in the `.env` file to enable text2gql search in InsightAgent.
...
Compared to the keyword search method, the text2gql search method can generate an accurate graph query laguage to query the entity of InsightAgent in knowledge graph, which is

```cypher
MATCH (n) WHERE n.id = 'InsightAgent' RETURN n LIMIT 10
```
