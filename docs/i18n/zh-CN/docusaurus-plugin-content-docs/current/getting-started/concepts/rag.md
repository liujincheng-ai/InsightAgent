...
RAG 通过在生成回答前从你自己的数据里检索相关上下文,来增强 LLM 的回答。InsightAgent 提供了完整的 RAG 框架,支持多种索引与检索策略,并把知识库对话跑成 **agentic RAG** 循环。
...
InsightAgent **不是**单次"检索-生成",而是由 agent 驱动循环:
