...
sidebar_position: 1
title: 前置条件
summary: "在本地或通过 Docker 运行 InsightAgent 前需要准备的环境"
read_when:
...
这里列出了安装 InsightAgent 之前需要准备的全部内容。
...
InsightAgent 需要 **Python 3.10+**。推荐使用 Python 3.11 以获得更好的兼容性。
...
从 v0.7.0 开始，InsightAgent 使用 [uv](https://docs.astral.sh/uv/) 进行环境与依赖管理，依赖解析更快也更稳定。
