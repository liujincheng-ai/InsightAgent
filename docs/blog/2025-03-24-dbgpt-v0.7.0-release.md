...
# InsightAgent V0.7.0, MCP + DeepSeek R1: Bringing More Possibilities to LLM Applications

**InsightAgent V0.7.0 Release: MCP Protocol Support, DeepSeek R1 Model Integration, Complete Architecture Upgrade, GraphRAG Retrieval Chain Enhancement, and More..**

`InsightAgent` is an open-source AI Native Data App Development framework with AWEL and Agents. In version `V0.7.0`, we have reorganized the `InsightAgent` module packages, splitting the original modules, restructuring the entire framework configuration system, and providing a clearer, more flexible, and more extensible management and development capability for building AI native data applications around large models.

## V0.7.0 version mainly adds and enhances the following core features

🍀 **Support for** `MCP(Model Context Protocol)` **protocol.**

🍀 **Integrated** `DeepSeek R1`, `QWQ` **inference models, all original** `InsightAgent` **Chat scenarios now cover deep thinking capabilities.**

🍀 `GraphRAG` **retrieval chain enhancement: support for "Vector" and "Intent Recognition+Text2GQL" graph retrievers.**

🍀 `InsightAgent` **module package restructuring, original** `dbgpt` **package split into** `dbgpt-core`, `dbgpt-ext`, `dbgpt-serve`, `dbgpt-client`, `dbgpt-acclerator`, `dbgpt-app`.

🍀 **Reconstructed** `InsightAgent` **configuration system, configuration files changed to "**`.toml`**" format, abolishing the original** `.env` **configuration logic.**
...
**And all Chat scenarios in original InsightAgent now have deep thinking capabilities.**

For quick usage reference: [https://liujincheng-ai.github.io/InsightAgent/docs/next/quickstart](https://liujincheng-ai.github.io/InsightAgent/docs/next/quickstart)
...
### 4. `InsightAgent` module package restructuring
...
For all configurations reference: [https://liujincheng-ai.github.io/InsightAgent/docs/next/config-reference/app/config_chatdashboardconfig_2480d0](https://liujincheng-ai.github.io/InsightAgent/docs/next/config-reference/app/config_chatdashboardconfig_2480d0)
...
InsightAgent unified storage extension OSS and S3 implementation, where the S3 implementation supports most cloud storage compatible with the S3 protocol. InsightAgent knowledge base original files, Chat Excel related intermediate files, AWEL Flow node parameter files, etc. all support cloud storage.
...
For detailed configuration instructions, please refer to: [https://liujincheng-ai.github.io/InsightAgent/docs/next/config-reference/utils/config_s3storageconfig_f0cdc9](https://liujincheng-ai.github.io/InsightAgent/docs/next/config-reference/utils/config_s3storageconfig_f0cdc9)
...
Currently, most models can be integrated on the InsightAgent page, with configuration information persistently saved and models automatically loaded when the service starts.
...
One-click deployment command for InsightAgent:
...
For details, see the documentation: [https://liujincheng-ai.github.io/InsightAgent/docs/next/installation/docker-build-guide](https://liujincheng-ai.github.io/InsightAgent/docs/next/installation/docker-build-guide)

### 15. **InsightAgent API compatible with OpenAI SDK**
...
[Overview | InsightAgent](https://liujincheng-ai.github.io/InsightAgent/docs/overview)
...
This version took nearly three months to develop and has been merged to the main branch for over a month. Hundreds of users participated in testing version 0.7.0, with GitHub receiving hundreds of issues feedback. Some users directly submitted PR fixes. The InsightAgent community sincerely thanks every user and contributor who participated in version 0.7.0!

## ✨Appendix
+ [Quick Start](https://liujincheng-ai.github.io/InsightAgent/docs/next/quickstart)
+ [Docker Quick Deployment](https://liujincheng-ai.github.io/InsightAgent/docs/next/installation/docker)