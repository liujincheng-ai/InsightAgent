...
- **SQLite users**: Back up your metadata database before upgrading. The new metadata tables and columns are created automatically when the InsightAgent service starts.
...
The v0.8.2 upgrade scripts are maintained in the repository under [`assets/schema/upgrade/v0_8_2/`](https://github.com/liujincheng-ai/InsightAgent/tree/main/assets/schema/upgrade/v0_8_2):
...
### Step 1 — Stop InsightAgent Service

Stop the running InsightAgent service using the same method you used to start it.
...
### Step 4 — Start InsightAgent Service

Restart the InsightAgent service using your preferred start method.
...
If the upgrade fails, stop the InsightAgent service and restore the metadata database from the backup taken before the upgrade. Because v0.8.2 changes metadata schema, do not continue running a partially upgraded database.
