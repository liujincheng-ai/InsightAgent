...
- **SQLite users**: Back up your metadata database before upgrading. The new metadata tables are created automatically when the InsightAgent service starts.
...
The v0.8.1 upgrade scripts are maintained in the repository under [`assets/schema/upgrade/v0_8_1/`](https://github.com/liujincheng-ai/InsightAgent/tree/main/assets/schema/upgrade/v0_8_1):
...
If the upgrade fails, stop the InsightAgent service and restore the metadata database from the backup taken before the upgrade. Because v0.8.1 changes metadata schema, do not continue running a partially upgraded database.
