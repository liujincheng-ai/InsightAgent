...
All the following operations are completed through the `dbgpt` command. To use the `dbgpt` command, you first need to install the `InsightAgent` project. You can install it through the following command
