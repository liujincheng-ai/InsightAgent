...
title: InsightAgent Docker Build Guide
sidebar_label: Docker Build Guide
description: Comprehensive guide for building InsightAgent Docker images with various configurations
keywords:
  - InsightAgent
  - Docker
  - Build
...
# InsightAgent Docker Build Guide

This guide provides comprehensive instructions for building InsightAgent Docker images with various configurations using the `docker/base/build_image.sh` script.

## Overview

The InsightAgent build script allows you to create Docker images tailored to your specific requirements. You can choose from predefined installation modes or customize the build with specific extras, environment variables, and other settings.
...
InsightAgent requires Python 3.10 or higher. The default is Python 3.11, but you can specify a different version:
...
You can run `uv run install_help.py list` in your local InsightAgent repository to see all available extras.

### Environment Variables

InsightAgent build supports environment variables for specialized builds. The main environment variable used is `CMAKE_ARGS` which is particularly important for Llama-cpp compilation.
...
### Enterprise InsightAgent with PostgreSQL and Elasticsearch
...
- [InsightAgent Documentation](https://github.com/liujincheng-ai/InsightAgent)
