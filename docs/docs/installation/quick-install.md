...
sidebar_position: 1
title: Quick Install
summary: "The fastest way to install InsightAgent with the installer script from the README"
read_when:
  - You want the shortest path to a working InsightAgent web UI
...
The fastest way to get InsightAgent running. The installer script prepares a local InsightAgent workspace, generates a provider profile, and gives you a ready-to-run webserver command.

## Recommended: installer script

Use the installer script if you want the shortest path from zero to a working InsightAgent web UI.

<CommandCopyCard command={`curl -fsSL https://raw.githubusercontent.com/liujincheng-ai/InsightAgent/main/scripts/install/install.sh | bash`} />
...
:::tip Best fit
Choose this path if you want to try InsightAgent quickly without managing the repository structure yourself.
:::
...
<CommandCopyCard command={`curl -fsSL https://raw.githubusercontent.com/liujincheng-ai/InsightAgent/main/scripts/install/install.sh \
...
Already have a local InsightAgent repository? Reuse it instead of cloning into `~/.dbgpt/InsightAgent`.
...
- a InsightAgent checkout under `~/.dbgpt/InsightAgent` unless `--repo-dir` is used
- generated provider configs under `~/.dbgpt/configs/`
- the InsightAgent home directory under `~/.dbgpt/`
- a ready-to-run webserver command using the generated profile

## Start InsightAgent after installation

After installation completes, start the webserver with the generated profile config:

<CommandCopyCard command={`cd ~/.dbgpt/InsightAgent && uv run dbgpt start webserver --profile <profile>`} />
...
### The install completed, but InsightAgent does not start cleanly
