...
InsightAgent will use all available gpu by default. And you can modify the setting `CUDA_VISIBLE_DEVICES=0,1` in `.env` file
...
InsightAgent supported 8-bit quantization and 4-bit quantization.
...
Note: you need to install the latest dependencies according to [requirements.txt](https://github.com/liujincheng-ai/InsightAgent/blob/main/requirements.txt).
Note: you need to install the latest dependencies according to [requirements.txt](https://github.com/liujincheng-ai/InsightAgent/blob/main/requirements.txt).