# 🚀 InsightAgent V0.8.2 — A More Complete Agentic Data Workflow: Multi-File Analysis, Knowledge Retrieval, and Parallel Sub-Agents
...
When a conversation with attachments is saved as a Scheduled Task, InsightAgent freezes task-scoped copies of those files. Each scheduled run can therefore replay against the same file snapshot instead of depending on mutable session uploads.
...
- Add session-scoped multi-file upload, preview, and Agentic analysis ([#3206](https://github.com/liujincheng-ai/InsightAgent/pull/3206))
- Add Agentic Knowledge-Base Search with index-method configuration and iterative RAG ([#3160](https://github.com/liujincheng-ai/InsightAgent/pull/3160))
- Add parallel sub-agent delegation and execution ([#3161](https://github.com/liujincheng-ai/InsightAgent/pull/3161))
- Add interactive human-in-the-loop questions to the built-in Agent tools ([#3107](https://github.com/liujincheng-ai/InsightAgent/pull/3107))
- Add the OrcaRouter proxy provider through an OpenAI-compatible endpoint ([#3186](https://github.com/liujincheng-ai/InsightAgent/pull/3186))
- Add an Alibaba Cloud MaxCompute (ODPS) datasource backed by the PyODPS SQLAlchemy dialect and configurable from the Web UI (Fixes [#3105](https://github.com/liujincheng-ai/InsightAgent/issues/3105)) ([#3178](https://github.com/liujincheng-ai/InsightAgent/pull/3178))

## Bug Fixes

- Avoid a `KeyError` when a chart or SQL run does not include `db_name` ([#3199](https://github.com/liujincheng-ai/InsightAgent/pull/3199))
- Include the flow UID in the `update_flow` PUT path (Fixes [#3193](https://github.com/liujincheng-ai/InsightAgent/issues/3193)) ([#3196](https://github.com/liujincheng-ai/InsightAgent/pull/3196))
- Use MySQL `LONGTEXT` for large Agent messages and action reports at the ORM layer ([#3189](https://github.com/liujincheng-ai/InsightAgent/pull/3189))
- Move `EXAMPLE_1` database creation to the end of the schema file ([#3183](https://github.com/liujincheng-ai/InsightAgent/pull/3183))
- Separate citations from final answers with a structured final-answer protocol ([#3182](https://github.com/liujincheng-ai/InsightAgent/pull/3182))
- Validate `user_id` and constrain resolved paths for Python file uploads (Fixes [#3104](https://github.com/liujincheng-ai/InsightAgent/issues/3104)) ([#3184](https://github.com/liujincheng-ai/InsightAgent/pull/3184))
- Load only files named exactly `SKILL.md` as Markdown Skills ([#3175](https://github.com/liujincheng-ai/InsightAgent/pull/3175))
- Prevent command injection in macOS TTS by removing shell interpretation (Fixes [#3129](https://github.com/liujincheng-ai/InsightAgent/issues/3129)) ([#3174](https://github.com/liujincheng-ai/InsightAgent/pull/3174))
- Handle LLM output containing multiple JSON code fences ([#3117](https://github.com/liujincheng-ai/InsightAgent/pull/3117))
- Render Agent Jinja2 templates in a sandboxed environment ([#3111](https://github.com/liujincheng-ai/InsightAgent/pull/3111))
- Stop `inner_copy_and_install` from reporting a failed build as successful ([#3141](https://github.com/liujincheng-ai/InsightAgent/pull/3141))
- Deserialize table chunks when separated chunks are present ([#3140](https://github.com/liujincheng-ai/InsightAgent/pull/3140))
- Use POST rather than GET in client `create_datasource` and `create_flow` calls ([#3138](https://github.com/liujincheng-ai/InsightAgent/pull/3138))
- Make CORS allowed origins configurable and correct wildcard/credential handling ([#3123](https://github.com/liujincheng-ai/InsightAgent/pull/3123))
- Honor negative read sizes and preserve the `SEEK_END` position in `StreamedBytesIO` ([#3136](https://github.com/liujincheng-ai/InsightAgent/pull/3136))
- Handle native Boolean values in `VariablesProvider._convert_to_value_type` ([#3135](https://github.com/liujincheng-ai/InsightAgent/pull/3135))
- Preserve bucket names in simplified fsspec paths ([#3134](https://github.com/liujincheng-ai/InsightAgent/pull/3134))
- Handle headerless sheets and multi-sheet workbooks in `ExcelKnowledge._load` ([#3137](https://github.com/liujincheng-ai/InsightAgent/pull/3137))
- Correct the empty/non-list result guard in `TeiRerankEmbeddings._parse_results` ([#3133](https://github.com/liujincheng-ai/InsightAgent/pull/3133))
- Avoid the `handleChat` temporal dead zone in the frontend ([#3132](https://github.com/liujincheng-ai/InsightAgent/pull/3132))
- Allow the Milvus vector-store type in TOML configuration ([#3127](https://github.com/liujincheng-ai/InsightAgent/pull/3127))
...
- [InsightAgent V0.8.1 Release Notes](https://liujincheng-ai.github.io/InsightAgent/docs/next/changelog/Released_V0.8.1)
...
- [RAG Concepts](https://liujincheng-ai.github.io/InsightAgent/docs/next/getting-started/concepts/rag)
- [Knowledge Base Indexing Principles](https://liujincheng-ai.github.io/InsightAgent/docs/next/design/kb_index_principles)
