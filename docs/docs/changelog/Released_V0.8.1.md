# 🚀 InsightAgent V0.8.1 — Production-Ready AI Data Assistant: Scheduling, Connectors, and Long-Running Agents
...
InsightAgent V0.8.0 moved the product experience from "conversational Q&A" to "task delivery." V0.8.1 focuses on the practical questions teams face when they use the AI Data Assistant continuously in real environments:
...
The core value of V0.8.1 is straightforward: InsightAgent AI Data Assistant moves from completing one complex analysis to supporting repeatable, long-running, team-ready workflows.
...
Run the analysis once, save the conversation as a task, and InsightAgent will replay the full ReAct Agent flow on a cron schedule. Each run creates a new conversation and report while preserving execution history for auditing, review, and team sharing.
...
V0.8.1 extends InsightAgent Agents beyond databases and local Skills through **MCP Connectors**. Agents can now access external tools through the Model Context Protocol, while users control which connectors are attached to each conversation.
...
| Encrypted credentials | Connector credentials are encrypted in InsightAgent metadata and restored after service restart |
...
These improvements make InsightAgent better suited for complex data analysis workflows that require multi-step reasoning, retries, and intermediate artifact management.

### 🧱 Broader Model, Datasource, and Storage Ecosystem

V0.8.1 expands the ecosystem around InsightAgent, making it easier for teams to reuse existing model, database, vector-store, and cache infrastructure.

#### LiteLLM Embedded AI Gateway

InsightAgent adds **LiteLLM** as an embedded proxy provider, registered as `proxy/litellm`. It is not a separate proxy service; InsightAgent calls LiteLLM in process, giving you a unified entry point to OpenAI, Anthropic, Vertex AI, Bedrock, Azure, Cohere, Mistral, Groq, Ollama, and other LiteLLM-supported backends.
...
- **Valkey vector client `CLIENT SETNAME`**: Make InsightAgent connections easier to identify in Valkey monitoring tools.
...
- Support Scheduled Tasks and MCP Connectors ([#3095](https://github.com/liujincheng-ai/InsightAgent/pull/3095))
- Add context management, task-plan tracking, and the corresponding frontend UI ([#3053](https://github.com/liujincheng-ai/InsightAgent/pull/3053))
- Add LiteLLM as an embedded AI gateway provider ([#3043](https://github.com/liujincheng-ai/InsightAgent/pull/3043))
- Add Qdrant vector search support ([#3034](https://github.com/liujincheng-ai/InsightAgent/pull/3034))
- Add Valkey vector store integration ([#3051](https://github.com/liujincheng-ai/InsightAgent/pull/3051))
- Add Valkey cache integration ([#3057](https://github.com/liujincheng-ai/InsightAgent/pull/3057))
- Add `CLIENT SETNAME` to the Valkey vector client ([#3090](https://github.com/liujincheng-ai/InsightAgent/pull/3090))
- Support configurable vector-search distance metrics ([#3044](https://github.com/liujincheng-ai/InsightAgent/pull/3044))
- Add TTL caching to `ConnectorManager.get_connector` (warm cache from about 63s to under 10ms) ([#3046](https://github.com/liujincheng-ai/InsightAgent/pull/3046))
- Add openGauss datasource support ([#3007](https://github.com/liujincheng-ai/InsightAgent/pull/3007))
- Implement `VARBINARY` and `BINARY` types for StarRocks ([#3062](https://github.com/liujincheng-ai/InsightAgent/pull/3062))
- Upgrade the MiniMax default model to M3 ([#3093](https://github.com/liujincheng-ai/InsightAgent/pull/3093))
- Harden `~/.dbgpt/configs/<profile>.toml` file permissions to `0o600` ([#3077](https://github.com/liujincheng-ai/InsightAgent/pull/3077))

## Bug Fixes

- Fix DuckDB datasource creation from the Web UI ([#3009](https://github.com/liujincheng-ai/InsightAgent/pull/3009))
- Support DeepSeek V4 Pro ([#3079](https://github.com/liujincheng-ai/InsightAgent/pull/3079))
- Use size chunking by default for Markdown knowledge (Fixes [#3030](https://github.com/liujincheng-ai/InsightAgent/issues/3030)) ([#3033](https://github.com/liujincheng-ai/InsightAgent/pull/3033))
- Restrict personal Skill script execution ([#3071](https://github.com/liujincheng-ai/InsightAgent/pull/3071))
- Validate example filenames ([#3066](https://github.com/liujincheng-ai/InsightAgent/pull/3066))
- Validate Skill upload filenames ([#3065](https://github.com/liujincheng-ai/InsightAgent/pull/3065))
- Constrain Python upload filenames ([#3064](https://github.com/liujincheng-ai/InsightAgent/pull/3064))
- Handle knowledge space ID responses ([#3070](https://github.com/liujincheng-ai/InsightAgent/pull/3070))
- Tolerate multi-step ReAct output ([#3074](https://github.com/liujincheng-ai/InsightAgent/pull/3074))
- Tell the LLM that the current table list is a top-K subset, not the full database table list ([#3045](https://github.com/liujincheng-ai/InsightAgent/pull/3045))
- Expand the `gpts_messages.content` column to hold longer Agent messages ([#3055](https://github.com/liujincheng-ai/InsightAgent/pull/3055))
- Make MilvusStore compatible with Milvus 2.5+ ([#3042](https://github.com/liujincheng-ai/InsightAgent/pull/3042))
- Add per-chunk fault tolerance and a per-database indexing lock ([#3040](https://github.com/liujincheng-ai/InsightAgent/pull/3040))
- Override MSSQL `get_fields()` with SQL Server-compatible `INFORMATION_SCHEMA` queries ([#3039](https://github.com/liujincheng-ai/InsightAgent/pull/3039))
- Add authentication dependencies to knowledge module APIs ([#3038](https://github.com/liujincheng-ai/InsightAgent/pull/3038))
- Fix BranchOperator incorrectly skipping shared downstream nodes (Fixes [#2935](https://github.com/liujincheng-ai/InsightAgent/issues/2935)) ([#3035](https://github.com/liujincheng-ai/InsightAgent/pull/3035))
- Honor the configured Tongyi embedding model name (Fixes [#3029](https://github.com/liujincheng-ai/InsightAgent/issues/3029)) ([#3032](https://github.com/liujincheng-ai/InsightAgent/pull/3032))
- Use explicit UTF-8 encoding when writing Code Interpreter temporary scripts ([#3023](https://github.com/liujincheng-ai/InsightAgent/pull/3023))
...
### Restart InsightAgent

Restart InsightAgent using your usual startup method. After startup, we recommend checking that:
...
- [InsightAgent V0.8.0 Release Notes](https://liujincheng-ai.github.io/InsightAgent/docs/next/changelog/Released_V0.8.0)
- [Quick Start](https://liujincheng-ai.github.io/InsightAgent/docs/overview/)
- [Installation Guide](https://liujincheng-ai.github.io/InsightAgent/docs/next/installation/)
- [Scheduled Tasks](https://liujincheng-ai.github.io/InsightAgent/docs/next/application/scheduled_tasks)
- [MCP Connectors](https://liujincheng-ai.github.io/InsightAgent/docs/next/application/mcp_connectors)
