...
InsightAgent uses a sandbox to let agents execute code and tools in an isolated runtime
...
In InsightAgent, a sandbox is an isolated execution environment used by an agent when it
...
real environments. The sandbox gives InsightAgent a dedicated runtime for actions such
as:
...
## InsightAgent's current sandbox solution

InsightAgent's sandbox implementation lives in:
...
This allows InsightAgent to prefer container isolation when available and fall back to a
local execution mode for development or environments without container support.

## Layered architecture in `dbgpt-sandbox`

InsightAgent's sandbox is implemented as a small runtime system with several layers.
...
One important part of the InsightAgent sandbox design is that it supports **session-based
stateful execution**.
...
## Current integration in InsightAgent app

Today, InsightAgent already uses sandbox execution in application-side agent tooling.
...
## What InsightAgent supports today

Based on the current `dbgpt-sandbox` implementation, InsightAgent is moving toward a
...
## High-level view of the current InsightAgent sandbox direction
