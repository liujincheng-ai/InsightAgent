...
In InsightAgent, a skill is a reusable capability package that gives an agent a structured way to solve a task.
...
<img
  ...
  alt="InsightAgent skills overview"
...
In InsightAgent terms, a skill is not just “something the model knows.” It is a packaged workflow that helps the agent decide:
...
A InsightAgent skill package often includes:
...
Following the structure used by InsightAgent's own skill-creator guidance, a skill is organized as a small self-contained package:
