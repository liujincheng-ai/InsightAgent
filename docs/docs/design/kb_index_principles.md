...
> How InsightAgent indexes an uploaded document — from raw file to searchable indexes.
...
> index and **code graph** are two extra shapes of indexing that InsightAgent builds on top of those.
...
During indexing, InsightAgent **persists** the heading path on each chunk but does **not** persist the tree.
