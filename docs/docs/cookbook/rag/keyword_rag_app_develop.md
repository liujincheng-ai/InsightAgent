...
In this example, we will show how to use the Full Text Search RAG framework in InsightAgent. Using traditional full-text search to implement RAG can, to some extent, alleviate the uncertainty and interpretability issues brought about by vector database retrieval.

You can refer to the python example file `InsightAgent/examples/rag/keyword_rag_example.py` in the source code. This example demonstrates how to load knowledge from a document and persist it in a full text store. Subsequently, it recalls knowledge relevant to your question by searching for keywords in the full text store.
...
Set variables below in `.env` file, let InsightAgent know how to connect to Full Text Search Engine Storage.
