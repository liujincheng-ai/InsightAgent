...
`model_name`. If the tokenizer cannot count the content, InsightAgent falls back to a
...
may quickly dominate the prompt. InsightAgent keeps the prompt compact by separating
...
If `AgentContext.output_dir` is set, InsightAgent uses that directory instead.
...
- Compaction is progressive: after each layer, InsightAgent recounts tokens and stops
