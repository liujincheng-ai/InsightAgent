...
- `DatasourceResource`: A resource that can be used to connect to various data sources in InsightAgent.
It just works when you run your agent in the InsightAgent environment(running in the InsightAgent webserver).

In previous sections [Agents With Database](../../introduction/database), we have introduced 
how to use the database resource in the InsightAgent agent, so you can refer to it for more details.
