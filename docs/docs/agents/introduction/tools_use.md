...
:::note
In InsightAgent agents, most LLMs support tool calls as long as their own capabilities are not too weak.
...
Actually, **tool** is a special **resource** in the `InsightAgent` agent. You will see more details in the [Resource](../modules/resource/resource.md) section.