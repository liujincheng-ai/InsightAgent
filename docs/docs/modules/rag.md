...
InsightAgent implements a **Multi-Source RAG (MS-RAG)** framework that goes beyond basic document Q&A. It supports multiple knowledge sources (documents, URLs, databases, knowledge graphs, git repos), multiple indexing strategies, and integrates deeply with the InsightAgent agent and workflow ecosystem. Conversation over a knowledge base is performed by an **agentic RAG** loop — the agent can rewrite the query, retrieve multiple times, fuse and rerank results, and produce a cited answer — rather than a single retrieve-then-generate pass.
...
InsightAgent selects which indexes to build per knowledge space via `index_methods` (a string list). Three index *methods* are persisted; **structural index** and **code graph** are two extra shapes layered on top. All indexes operate on the *same chunks*, so chunking quality dominates retrieval quality.
...
When a user asks a question over a knowledge base, InsightAgent does **not** do a single retrieve-then-generate. Instead an **agent** drives the loop:
...
This agentic loop — multi-step retrieval, query rewriting, result fusion and reranking, and citation — is what lets InsightAgent answer complex or multi-part questions that a one-shot RAG cannot. The full flow is documented in [Agentic RAG Conversation Principles](/docs/design/agentic_rag_principles).
...
InsightAgent supports loading knowledge from multiple source types. In the Web UI, you can select a datasource type when uploading:
...
When the **KnowledgeGraph** index method is enabled, InsightAgent builds a **family of graphs**, not a single one. They share one build path and all support edge-traversal retrieval:
...
Document chunking is a critical step in RAG quality — it is the shared foundation under every index. InsightAgent supports multiple chunking strategies:
...
InsightAgent supports a wide range of embedding models for converting text into vector representations:
...
You can configure the retrieval strategy for your knowledge base. InsightAgent supports multiple retrieve modes — **Semantic**, **Keyword**, **Hybrid**, and **Tree** — to suit different query scenarios. Select the mode that best fits your use case in the knowledge base settings.
...
| Source Code | [GitHub](https://github.com/liujincheng-ai/InsightAgent/tree/main/packages/dbgpt-core/src/dbgpt/rag) |