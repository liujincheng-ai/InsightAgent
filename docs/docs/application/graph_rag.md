...
Graph RAG is the retrieval mode that uses InsightAgent's **knowledge-graph index**. When you enable the `KnowledgeGraph` index method on a knowledge space, InsightAgent builds a *family of graphs* and retrieves by graph traversal instead of (or in addition to) vector / keyword search.
