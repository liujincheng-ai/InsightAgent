...
First, select the [data source](../datasources.md) on the left to add and add a database. Currently, InsightAgent supports multiple database types. Just select the corresponding database type to add. Here we choose MySQL as a demonstration. For the test data of the demonstration, see the [test sample](https://github.com/liujincheng-ai/InsightAgent/tree/main/docker/examples/sqls).
