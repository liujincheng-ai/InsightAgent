...
summary: "Run InsightAgent from source with uv, configure a provider, and verify the webserver"
read_when:
...
Deploy InsightAgent directly from source code. This is the most flexible option for development, debugging, and custom integrations.
...
InsightAgent provides an interactive helper to generate the right `uv sync` command:
...
:::tip Verify it works
If the Web UI loads and you can start a chat conversation, your InsightAgent is running.
:::
