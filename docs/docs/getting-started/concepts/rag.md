...
RAG enhances LLM responses by retrieving relevant context from your own data before generating an answer. InsightAgent provides a comprehensive RAG framework with multiple indexing and retrieval strategies, and runs knowledge-base chat as an **agentic RAG** loop.
...
InsightAgent does **not** do a single retrieve-then-generate. An agent drives the loop:
...
1. Open the InsightAgent Web UI
