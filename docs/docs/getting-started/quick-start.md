...
summary: "Shortest path from clone to a working InsightAgent chat"
read_when:
  - You want the first successful InsightAgent run with the least setup
...
:::info Fastest path
Use an **API proxy** (OpenAI or DeepSeek) — no GPU required. You will have a working InsightAgent chat in under 5 minutes.
:::
...
:::tip Verify it works
If the Web UI loads and you can start a chat conversation, your InsightAgent is ready for use.
:::
...
- **Web UI does not load**
  ...
  - Check the server logs in the terminal where you started InsightAgent
