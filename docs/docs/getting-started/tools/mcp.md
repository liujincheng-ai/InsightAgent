...
The **Model Context Protocol (MCP)** enables InsightAgent agents to connect with external tools and services through a standardized interface.

:::info What is MCP?
MCP is an open protocol that provides a standard way for AI applications to connect with external data sources and tools. InsightAgent supports MCP as both a **client** (consuming MCP tools) and a **server** (exposing InsightAgent capabilities as MCP tools).
:::
...
```mermaid
graph TB
  Agent[InsightAgent Agent] --> MCPClient[MCP Client]
  ...
  ExtClient[External MCP Client] --> MCPServer[InsightAgent MCP Server]
  MCPServer --> DBGPT[InsightAgent Capabilities]
```
...
## InsightAgent as an MCP server

InsightAgent can also expose its capabilities as an MCP server, allowing other MCP-compatible applications to use InsightAgent features like:
