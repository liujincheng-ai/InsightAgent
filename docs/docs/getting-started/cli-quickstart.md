...
Install InsightAgent from PyPI and start it with a single command — no source checkout required.
...
That's it! On first run InsightAgent will launch an **interactive setup wizard** that helps you:
...
Commands:
  start     Start the InsightAgent server
...
Start the InsightAgent web server. Running `dbgpt start` without a subcommand is equivalent to `dbgpt start web`.
...
Stop running InsightAgent server processes.
...
After first run, InsightAgent creates the following structure under your home directory:
...
|---|---|---|
| `DBGPT_HOME` | `~/.dbgpt` | Override the InsightAgent home directory |
