...
_SECTION_PATTERN = re.compile(r"(?:^|[-\s])([1-9]\d*(?:\.\d+)+)(?:\s|$)")


def _primary_section(value: Any) -> str | None:
    """Extract a section id from stable chunk metadata when it is available."""

    if not isinstance(value, str):
        return None
    match = _SECTION_PATTERN.search(value)
        ...
        try:
            ...
            if chunks:
                ...
                for i, chunk in enumerate(chunks[:5]):
                    ...
                    # Chunk names are produced during document splitting and
                    # retain the retrieval hit's section identity.  Persist it
                    # only when present, preserving the established wire shape
                    # for unsectioned documents.
                    primary_section = _primary_section(
                        metadata.get("section")
                        or metadata.get("section_name")
                        or getattr(chunk, "chunk_name", None)
                    )
                    if primary_section:
                        citation["primarySection"] = primary_section
