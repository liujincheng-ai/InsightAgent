...
def test_database_and_knowledge_selection_is_not_a_knowledge_only_turn() -> None:
    dialogue = SimpleNamespace(
        ext_info={
    ...
    assert agentic_data_api._is_knowledge_turn(dialogue, "full") is False
    assert agentic_data_api._is_knowledge_turn(SimpleNamespace(), "full") is False
    assert agentic_data_api._is_knowledge_turn(SimpleNamespace(), "knowledge") is True
...
async def test_turn_timeout_emits_diagnostic_final_and_done(monkeypatch) -> None:
    ...
    monkeypatch.setenv("DBGPT_REACT_AGENT_TIMEOUT_SECONDS", "0.01")
    monkeypatch.setattr(
        agentic_data_api,
        "_react_agent_stream_impl",
        _slow_stream_impl,
    ...
    events = [
        ...
        async for event in agentic_data_api._react_agent_stream(
            SimpleNamespace(conv_uid="timeout-case")
        )
    ]

    assert [event["type"] for event in events] == ["diagnostic", "final", "done"]
    assert events[0]["code"] == "REACT_TURN_TIMEOUT"
    assert events[0]["conv_id"] == "timeout-case"
    assert "已安全终止" in events[1]["content"]
