...
@pytest.mark.asyncio
async def test_knowledge_retrieve_persists_chunk_primary_section() -> None:
    class _SectionedResource:
        async def retrieve(self, query: str):
            return [
                SimpleNamespace(
                    chunk_id="discount-4-2",
                    chunk_name="制度-4.2 超标准折扣审批",
                    content="超过标准折扣时必须分级审批。",
                    score=0.9,
                    metadata={"doc_name": "汽车配件价格与折扣管理办法.md"},
                )
            ]

    retrieve = make_knowledge_retrieve({}, [_SectionedResource()])
    result = json.loads(await retrieve("折扣审批"))

    assert result["citations"][0]["primarySection"] == "4.2"
