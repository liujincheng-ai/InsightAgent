...
from .citation_followup import build_citation_followup_answer, is_citation_followup
...
def _insight_reliability_config(
    ext_info: Optional[Dict[str, Any]],
) -> Dict[str, Any]:
    """Build request-local reliability config and gate fault injection."""
    ...
    case_id = str(ext_info.get("reliability_case_id") or "").strip()
    if not case_id:
        return config
    if os.getenv("INSIGHT_AGENT_RELIABILITY_EVAL") != "1":
        logger.warning(
            "Ignored reliability fault case because evaluation mode is disabled"
        )
        return config
    profile = str(ext_info.get("reliability_profile") or "after").strip().lower()
    if profile not in {"baseline", "after"}:
        raise ValueError("reliability_profile must be baseline or after")
    from insight_agent import __file__ as insight_package_file
    from insight_agent.evaluation.reliability_schemas import (
        load_reliability_dataset,
        reliability_case_by_id,
        validate_reliability_dataset,
    )

    dataset_path = (
        Path(insight_package_file).resolve().parent
        / "evaluation"
        / "dataset"
        / "failure_cases.json"
    )
    ...
    if errors:
        raise ValueError("Week 4 reliability dataset is invalid: " + "; ".join(errors))
    case = reliability_case_by_id(dataset, case_id)
    if case is None:
        raise ValueError(f"Unknown Week 4 reliability case: {case_id}")
    requested_max_steps = ext_info.get("reliability_max_steps")
    max_steps = (
        int(requested_max_steps)
        if requested_max_steps is not None
        else int(case["max_steps"])
    )
    return {
        "profile": profile,
        "policy_enabled": profile == "after",
        "fault_plan": list(case["fault_plan"]),
        "case_id": case_id,
...
async def _load_context_budget_config(
    ...
    try:
        ...
        profile_name = (profile or "baseline").strip().lower()
        profile_overrides: Dict[str, Dict[str, Any]] = {
            ...
            "balanced": {
                ...
                "error_threshold": 0.86,
                "critical_threshold": 0.94,
                "min_keep_recent_rounds": 5,
                "max_observation_age_rounds": 4,
                "truncated_observation_max_chars": 1500,
                "min_keep_tokens": 20000,
                "tool_result_threshold": 12000,
                "turn_result_budget": 24000,
                "preview_size": 1200,
                "tool_overrides": {"sql_query": 8000, "semantic_search": 8000},
            },
            "aggressive": {
                "max_context_tokens": 18000,
                "reserved_tokens": 2048,
                "warning_threshold": 0.45,
                "error_threshold": 0.78,
                "critical_threshold": 0.90,
                "min_keep_recent_rounds": 2,
                "max_observation_age_rounds": 1,
                "truncated_observation_max_chars": 500,
                "min_keep_tokens": 0,
                "tool_result_threshold": 4000,
                "turn_result_budget": 8000,
                "preview_size": 750,
                "tool_overrides": {"sql_query": 3000, "semantic_search": 4000},
            },
        }
        selected = profile_overrides.get(profile_name, {})

        def _profile_value(field_name: str, default: Any) -> Any:
            if field_name in selected:
                return selected[field_name]
            return _value(agent_context, field_name, default)

        return ContextBudgetConfig(
            max_context_tokens=selected.get("max_context_tokens", max_context_tokens),
            warning_threshold=_profile_value(
                "warning_threshold", defaults.warning_threshold
            ),
            error_threshold=_profile_value("error_threshold", defaults.error_threshold),
            critical_threshold=_profile_value(
                "critical_threshold", defaults.critical_threshold
            ),
            reserved_tokens=_profile_value("reserved_tokens", defaults.reserved_tokens),
            min_keep_recent_rounds=_profile_value(
                "min_keep_recent_rounds",
                defaults.min_keep_recent_rounds,
            ),
            max_compact_failures=_profile_value(
                "max_compact_failures",
                defaults.max_compact_failures,
            ),
            max_observation_age_rounds=_profile_value(
            ...
            min_keep_tokens=_profile_value(
                "min_keep_tokens",
                defaults.min_keep_tokens,
            ),
            tool_result_threshold=_profile_value(
                "tool_result_threshold", defaults.tool_result_threshold
            ),
            turn_result_budget=_profile_value(
                "turn_result_budget", defaults.turn_result_budget
            ),
            preview_size=_profile_value("preview_size", defaults.preview_size),
            tool_overrides=_profile_value("tool_overrides", defaults.tool_overrides),
        )
    except Exception:
        logger.debug(
...
def _is_knowledge_turn(dialogue: Any, tool_mode: str) -> bool:
    """Recognize both dedicated and generic ReAct knowledge-base requests."""
    ...
    if not isinstance(ext_info, dict):
        return False
    has_knowledge = bool(
        ext_info.get("knowledge_space")
        or ext_info.get("knowledge_space_name")
        or ext_info.get("knowledge_space_id")
    )
    # A database and knowledge base selected together is a combined analysis
    # request, not a pure knowledge chat.  Treating it as knowledge-only hides
    # all registered business tools from the native ReAct Action Space.
    return has_knowledge and not bool(ext_info.get("database_name"))


def _is_insight_integrated_request(
    database_connector: Any,
    knowledge_space: Optional[str],
    database_name: Optional[str],
) -> bool:
    """Recognize InsightAgent DB + knowledge requests regardless of UI tool mode.

    The Web client may label a turn as ``knowledge`` when a knowledge base is
    selected even though it also sends a database.  The selected resources are
    authoritative; requiring the client hint to be ``full`` incorrectly hides
    all native business Tools.
    """

    return bool(
        database_connector is not None
        and knowledge_space
        and is_insight_database(database_name)
    )


def _extract_sql_candidate(value: Any) -> Optional[str]:
    """Recover a lone SQL mapping without evaluating model-generated code."""
    ...
    try:
        ...
        try:
            ...
            return None
    if not isinstance(parsed, dict) or set(parsed) != {"sql"}:
        return None
    sql = parsed.get("sql")
    return sql.strip() if isinstance(sql, str) and sql.strip() else None


def _extract_search_candidate(value: Any) -> Optional[str]:
    """Recover only a lone semantic-search query mapping."""
...
async def _react_agent_stream(
    ...
    """
    inner_stream = _react_agent_stream_inner(dialogue, tool_mode, attachment_ctx)
    try:
        ...
        try:
            ...
            logger.warning("Failed to close owned ReAct stream", exc_info=True)
...
async def _react_agent_stream_inner(
    ...
    reliability_runtime: Dict[str, Any] = {}
    final_emitted = False
    done_emitted = False
    is_knowledge_turn = _is_knowledge_turn(dialogue, tool_mode)
    timeout_env = os.getenv("DBGPT_REACT_AGENT_TIMEOUT_SECONDS")
    try:
        timeout_seconds = (
            float(timeout_env)
            if timeout_env is not None
            else (90.0 if is_knowledge_turn else 120.0)
        )
    except ValueError:
        timeout_seconds = 90.0 if is_knowledge_turn else 120.0
    timeout_seconds = max(0.01, timeout_seconds)
    impl_stream: Optional[AsyncGenerator[str, None]] = None
    try:
        async with asyncio.timeout(timeout_seconds):
            ...
            if attachment_ctx is not None:
                stream_kwargs["attachment_ctx"] = attachment_ctx
            impl_stream = _react_agent_stream_impl(dialogue, **stream_kwargs)
        ...
        conv_id = dialogue.conv_uid or "unknown"
        logger.error(
            "ReAct turn timed out conv_id=%s mode=%s timeout_seconds=%s",
            conv_id,
            "knowledge" if is_knowledge_turn else tool_mode,
            timeout_seconds,
        )
        yield _sse_event(
            {
                "type": "diagnostic",
                "code": "REACT_TURN_TIMEOUT",
                "phase": "knowledge" if is_knowledge_turn else tool_mode,
                "conv_id": conv_id,
                "timeout_seconds": timeout_seconds,
            }
        )
        timeout_final_content = (
            f"本次任务在 {timeout_seconds:g} 秒内未完成，已安全终止。"
            f"诊断编号：{conv_id}。"
        )
        reliability_tool_pack = reliability_runtime.get("tool_pack")
        if reliability_tool_pack is not None:
            from insight_agent.agent import (
            ...
            if (
                isinstance(reliability_tool_pack, InsightReliabilityToolPack)
                and reliability_tool_pack.policy_enabled
            ):
                reliability_tool_pack.record_terminal_timeout(
                    message=(
                        f"ReAct turn exceeded the {timeout_seconds:g}-second "
                        "request limit"
                    )
                )
                completion = reliability_tool_pack.completion_result()
                yield _sse_event(
                    {
                        "type": "reliability.trace",
                        "case_id": reliability_runtime.get("case_id"),
                        "profile": reliability_runtime.get("profile"),
                        "max_steps": reliability_runtime.get("max_steps"),
                        "completion": completion.to_dict(),
                        "decisions": reliability_tool_pack.diagnostics,
                    }
                )
                timeout_final_content = completion.to_user_text()
                if isinstance(reliability_tool_pack, SalesDiagnosisFirstToolPack):
                    verified_partial = format_focused_domain_answer(
                        reliability_tool_pack.required_domain_tool,
                        reliability_tool_pack.last_domain_result,
                        str(dialogue.user_input or ""),
                    )
                    if verified_partial:
                        timeout_final_content = (
                            "以下数据结果已经完成并通过 Tool Observation 验证：\n"
                            + verified_partial
                            + "\n\n"
                            + timeout_final_content
                        )
        yield _sse_event(
            AgentFinalAnswer(content=timeout_final_content).to_sse_payload()
        )
        ...
        if impl_stream is not None:
            try:
                ...
                logger.warning(
                    "Failed to close ReAct implementation stream", exc_info=True
                )
...
async def _react_agent_stream_impl(
    ...
    reliability_runtime: Optional[Dict[str, Any]] = None,
) -> AsyncGenerator[str, None]:
    ...
    reliability_config = _insight_reliability_config(dialogue.ext_info)
    reliability_diagnostics: List[Dict[str, Any]] = []
...
""".format(file_path=react_state["file_path"])
    ...
    # Citation-only follow-ups do not need another retrieval/planning loop when
    # the previous rounds already persisted structured citations.  This makes
    # the answer deterministic and removes the former indefinite Delegating path.
    is_knowledge_turn = tool_mode == "knowledge" or bool(knowledge_space)
    if is_knowledge_turn and is_citation_followup(user_input):
        citation_answer = build_citation_followup_answer(storage_conv.messages)
        if citation_answer is not None:
            logger.info(
                "Resolved citation follow-up from history conv_id=%s sources=%d",
                conv_id,
                len(citation_answer.citations),
            )
            history_payload = _build_react_history_payload(
                final_content=citation_answer.content,
                steps=[],
                task_plan=[],
                generated_images=[],
                sub_agents=[],
                input_files=input_files_snapshot,
                citations=[item.to_dict() for item in citation_answer.citations],
            )
            for terminal_event in _react_terminal_events(
                storage_conv, history_payload, citation_answer
            ):
                yield terminal_event
            return
        logger.warning(
            "Citation follow-up history was incomplete conv_id=%s messages=%d",
            conv_id,
            len(storage_conv.messages),
        )

    tool_calling_profile = (
        str(dialogue.ext_info.get("tool_calling_profile") or "").strip().lower()
        if isinstance(dialogue.ext_info, dict)
        else ""
    )
    is_tool_calling_evaluation = bool(tool_calling_profile)
    is_database_fast_path = bool(
        database_connector is not None
        and not knowledge_space
        and not file_path
        and pre_matched_skill is None
        and not connector_ids
        and not is_tool_calling_evaluation
    )
    is_insight_integrated_mode = (
        not is_tool_calling_evaluation
        and _is_insight_integrated_request(
            database_connector,
            knowledge_space,
            database_name,
        )
    )
    is_knowledge_fast_path = is_knowledge_turn and not is_insight_integrated_mode
    default_temperature = (
        0.0
        if (
            is_database_fast_path
            or is_knowledge_fast_path
            or is_tool_calling_evaluation
        )
        else 0.2
    )
    context = AgentContext(
        ...
        gpts_app_name="ReAct",
        language="zh",
        # Focused SQL/RAG evaluation paths must be deterministic even when the
        # web client sends its generic 0.6 conversation default.
        temperature=(
            default_temperature
            if (
                is_database_fast_path
                ...
                if dialogue.temperature is not None
...
"""
    ...
    if is_skill_mode:
        ...
        # html_interpreter
        workflow_prompt = f"""
You are the InsightAgent intelligent assistant, executing the skill task selected by the user.
...
""".strip()
        ...
        workflow_prompt = f"""
You are the InsightAgent intelligent assistant, capable of autonomously selecting tools
...
""".strip()
    ...
    if is_tool_calling_evaluation:
        from insight_agent.tools import build_business_tools

        business_eval_tools = build_business_tools(tool_calling_profile)
        # Keep SQL available for the negative/control cases, but make its domain
        # boundary explicit.  The production sql_query instance is request-local,
        # so this description change cannot leak into normal routing.
        sql_query_tool._tool._description = (
            "仅用于普通销售额、销量、计数、简单排名等无需领域规则的只读 SELECT 查询。"
            "只要问题要求销售变化归因、产品或渠道贡献、客户停止采购或流失风险、"
            "经销商健康分或扣分原因，就不得使用 sql_query，必须调用对应业务 Tool。"
            '参数: {"sql": "SELECT 语句"}'
        )
        workflow_prompt = f"""
你是 InsightAgent 的 Tool Calling 专项评测代理。当前配置为
`{tool_calling_profile}`。你必须根据用户意图和可用 Tool 契约自主选择动作，系统不会
预先指定业务 Tool，也不会在漏调后自动恢复。
...
Action 必须严格取自：{{{{ action_space_names }}}}。不得缩写、改名或虚构 Tool。

## 决策规则
1. 销售变化原因、产品或渠道贡献使用最匹配的销售诊断 Tool；客户停止采购、流失
   风险名单或客户下降贡献使用客户流失 Tool；经销商健康分、风险排序或扣分原因
   使用经销商健康 Tool。具体边界以每个 Tool 的 description 和参数 Schema 为准。
2. 普通销售额、销量、数量、简单排名或不需要领域规则的聚合问题使用 `sql_query`，
   不得为了展示能力而强行调用业务 Tool。
3. 一个问题包含两个独立领域意图时，调用覆盖这些意图所需的最少业务 Tool；组合题
   也必须每轮只输出并调用一个 Tool，收到 Observation 后才能在下一轮调用第二个。
   严禁在同一个 Action Input 后追加第二组 Action。每个 Tool 与同一参数最多调用一次，
   不得调用题目未要求的额外业务 Tool。
   数量词只作用于它直接修饰的结果列表。例如“销售归因，并列出前 4 名风险客户”中，
   `top_n=4` 只属于客户 Tool；销售 Tool 没有单独数量要求时必须使用其默认 `top_n=5`。
4. 参数必须来自用户问题；不得把 `current_quarter` 改成其他字段名。缺少会改变结果
   的必填参数时直接 terminate 提出澄清，不得猜测。
5. Tool 返回 `invalid_arguments` 时，只允许根据结构化 details 修正一次；再次失败则
   terminate 说明无法执行。任何参数错误都不得绕过校验进入数据库。
6. 获得足够 Observation 后立即 terminate，最终答案必须与 Observation 一致。

每轮严格输出一组 Thought、Action、Action Input；Action Input 必须是 JSON 对象。
禁止只输出参数字典。以下两种格式均可，除此之外不要输出协议字段：

Thought: 简述选择理由
Action: sales_diagnosis_tool
Action Input: {{"region": "华东", "category": "刹车系统", "current_quarter": "2026Q2"}}

或单个 JSON 对象：
{{"Thought": "简述选择理由", "Action": "sales_diagnosis_tool", "Action Input": {{"region": "华东", "category": "刹车系统", "current_quarter": "2026Q2"}}}}
""".strip()
        # Business tools come first so the compact action-space summary presents
        # the domain contracts before the generic SQL escape hatch.
        tool_pack = ToolPack([*business_eval_tools, sql_query_tool, Terminate()])
    elif is_database_fast_path:
        # A database-only question should not drift into todo planning,
        # sub-agents, Python, shell or HTML generation.  A focused two-tool
        # surface is both faster and easier to evaluate.
        semantic_profile = (
            dialogue.ext_info.get("semantic_profile")
            if isinstance(dialogue.ext_info, dict)
            else None
        )
        workflow_prompt = build_database_agent_prompt(
            database_context, user_input, semantic_profile
        )
        semantic_decision = build_semantic_context(user_input, semantic_profile)
        if semantic_decision.requires_clarification:
            react_state["semantic_clarification_questions"] = list(
                semantic_decision.clarification_questions
            )
        tool_pack = (
            ...
            if semantic_decision.requires_clarification
            else InsightReliabilityToolPack(
                [sql_query_tool, Terminate()],
                reliability_config=reliability_config,
                reliability_diagnostics=reliability_diagnostics,
            )
        )
    elif is_knowledge_fast_path:
        # Knowledge chat has the same focused contract.  In particular, citation
        # follow-ups should terminate from history or after minimal retrieval,
        # never create a task plan that can remain stuck at Delegating.
        workflow_prompt = f"""
你是只读企业知识库问答代理，只能根据检索到的制度内容回答。

{knowledge_context}

## 执行规则
1. 简单问题最多调用 3 次知识库工具；获得足够证据后立即 terminate，禁止
   使用完全相同的参数重复调用同一工具。
2. 不创建任务清单，不委派子任务，不生成 HTML，也不调用数据库工具。
3. 第一轮优先 semantic_search，参数为 {{"query": "完整用户问题"}}；证据不足时
   才补一次 kb_grep。读取已知文件必须使用 kb_cat，参数名是 path，例如
   {{"path": "制度文件名.md"}}，不能写成 file 或 filename。
4. 回答必须写明制度文件名和章节；先定位命中章节并逐条核对。
   “谁负责”类问题须列全该章节的责任主体及职责；措施、审批类问题须覆盖
   该章节全部关键时限、阈值和禁止项。不能从证据确认时明确说明。
5. 对“以上/上述答案来自哪里”的追问，优先使用会话历史已有引用，
   不重复执行无关检索。
6. 每轮只输出一个动作。Action Input 必须是完整 JSON 对象，不得放在
   Markdown 代码块中。完成时使用：
   Action: terminate
   Action Input: {{"result": "带制度与章节的最终中文答案"}}

## 可用工具
- kb_ls、kb_glob、kb_grep、kb_cat、semantic_search：只读知识库工具
- terminate：返回最终答案
""".strip()
        tool_pack = InsightReliabilityToolPack(
            kb_tool_list + [Terminate()],
            reliability_config=reliability_config,
            reliability_diagnostics=reliability_diagnostics,
        )

    if is_insight_integrated_mode:
        # Use a focused prompt as well as a focused tool surface.  Merely
        # removing tools from ToolPack is insufficient when the generic prompt
        # still advertises load_tools/dispatch/code actions that are no longer
        # available: the model may spend its bounded steps calling ghosts.
        workflow_prompt = f"""
你是 InsightAgent 汽车配件经营分析代理，只处理已选择的销售数据库与企业制度库。

{database_context}
{knowledge_context}

{build_integrated_business_prompt(user_input)}

## 可用动作
- sales_diagnosis_tool、customer_loss_analysis_tool、dealer_health_analysis_tool
- semantic_search、kb_grep、kb_cat、kb_ls、kb_glob
- sql_query（仅领域 Tool 明确 no_data/error 后补充查询）
- html_interpreter、read_file、terminate

禁止调用或提及 load_tools、execute_tool、dispatch_parallel_tasks、question、代码或
Shell 工具。每轮只能执行一个可用动作；收到 Observation 后再决定下一步。
严格输出 Thought、Action Intention、Action Reason、Action、Action Input；
Action Input 必须是 JSON 对象。完成时必须调用 terminate 返回最终中文答案。
""".strip()
        # Keep the ResourceManager registration as the source of truth, but
        # also make the native business tool explicit in this combined Web
        # ToolPack.  This protects against startup ordering where the manager
        # registry is populated after the request resource list is assembled.
        ...
        # Domain Tools are already first-class resources. Remove indirect tool
        # discovery/execution wrappers so the model cannot hide a domain call
        # behind a generic action or invent a different parameter schema.
        allowed_integrated_tools = {
            "customer_loss_analysis_tool",
            "dealer_health_analysis_tool",
            "html_interpreter",
            "kb_cat",
        ...
        def _resource_name(resource: Any) -> Optional[str]:
            direct_name = getattr(resource, "name", None)
            if direct_name:
                return str(direct_name)
            wrapped_tool = getattr(resource, "_tool", None)
            wrapped_name = getattr(wrapped_tool, "name", None)
            return str(wrapped_name) if wrapped_name else None

        integrated_resources = [
            resource
            for resource in tool_pack.sub_resources
            if _resource_name(resource) in allowed_integrated_tools
        ]
        domain_tools = (
            sales_diagnosis_tool,
            customer_loss_analysis_tool,
        ...
        existing_names = {_resource_name(resource) for resource in integrated_resources}
        integrated_resources.extend(
            domain_tool
            for domain_tool in domain_tools
            if _resource_name(domain_tool) not in existing_names
        )
        normalized_integrated_question = str(user_input or "").replace(" ", "")
        ...
        for tool_name, markers in (
            ...
            if any(marker in normalized_integrated_question for marker in markers):
                requested_domain_tools.append(tool_name)
        tool_pack = SalesDiagnosisFirstToolPack(
            integrated_resources,
            required_domain_tool=select_domain_tool(user_input),
            required_domain_tools=requested_domain_tools,
            require_knowledge_and_report=requires_full_business_report(user_input),
            reliability_config=reliability_config,
            reliability_diagnostics=reliability_diagnostics,
        )
    ...
    if isinstance(tool_pack, InsightReliabilityToolPack) and tool_pack.policy_enabled:
        workflow_prompt += """

## InsightAgent 失败恢复合同
1. Tool 返回 `next_action=retry` 时只允许修正参数或查询后重试一次，禁止原样重复。
2. Tool 返回 `next_action=replan` 时必须更换查询范围、检索方式或合适的 Tool；
   如果换策略仍失败，立即终止。
3. `retryable=false`、权限拒绝或 `CIRCUIT_BREAKER_OPEN` 必须立即 terminate，
   不得绕过保护调用其他工具获取同一受限数据。
4. 若已有经过验证的 Observation，后续步骤失败时保留可用结果，并明确已完成、
   未完成、失败原因和下一步；不得把部分完成描述为完整成功。
5. 制度检索失败时不得凭常识补写制度建议，数据库失败时不得从制度文本推断数值。
""".strip()
    ...
    max_retry_count = (
        5
        if is_database_fast_path
        else (
            6
            if is_knowledge_fast_path
            else (
                8
                if is_tool_calling_evaluation
                else (5 if is_insight_integrated_mode else 30)
            )
        )
    )
    if reliability_config.get("case_id"):
        max_retry_count = int(reliability_config["max_steps"])
    if reliability_runtime is not None:
        reliability_runtime.update(
            {
                "tool_pack": tool_pack,
                "case_id": reliability_config.get("case_id"),
                "profile": reliability_config.get("profile"),
                "max_steps": max_retry_count,
            }
        )
    agent_builder = (
        ReActAgent(max_retry_count=max_retry_count)
        .bind(context)
        .bind(agent_memory)
        .bind(llm_config)
        .bind(tool_pack)
    ...
    agent.init_context_management(
        config=await _load_context_budget_config(
            ...
            profile=(
                dialogue.ext_info.get("context_profile")
                if isinstance(dialogue.ext_info, dict)
                else None
            ),
        ),
    ...
    initial_integrated_plan: List[Dict[str, str]] = []
    if is_insight_integrated_mode:
        initial_integrated_plan = [
            {
                "content": f"调用 {tool_pack.required_domain_tool} 获取经营事实",
                "status": "in_progress",
                "priority": "high",
            },
            {
                "content": "检索企业制度依据并核对适用章节",
                "status": "pending",
                "priority": "high",
            },
            {
                "content": "生成证据绑定的 HTML 经营分析报告",
                "status": "pending",
                "priority": "high",
            },
        ]
        _todo_list.extend(initial_integrated_plan)
    ...
    if initial_integrated_plan:
        yield _sse_event(
            {
                "type": "plan.update",
                "tasks": list(initial_integrated_plan),
                "source": "insight_integrated_workflow",
            }
        )
    ...
    # Some models occasionally return only a Python-style {"sql": "..."}
    # mapping instead of the required ReAct Action envelope. Recover that one
    # narrow shape through the normal validated sql_query Tool. No arbitrary
    # code is evaluated, and all regular read-only/retry guards still apply.
    if (
        is_database_fast_path
        and is_insight_database(database_name)
        and react_state.get("last_sql_result") is None
        and not (
            isinstance(tool_pack, InsightReliabilityToolPack)
            and tool_pack.force_terminal
        )
    ):
        raw_sql_candidate = (reply.content or "") if reply is not None else ""
        if reply.action_report and reply.action_report.content:
            raw_sql_candidate = reply.action_report.content
        sql_candidate = _extract_sql_candidate(raw_sql_candidate)
        if sql_candidate:
            recovered_sql_text = await run_in_threadpool(sql_query_tool, sql_candidate)
            recovered_payload = json.loads(recovered_sql_text)
            recovered_status = (
                "done" if recovered_payload.get("status") == "success" else "failed"
            )
            sql_step_id, sql_step_start = build_step(
                "sql_query",
                "缺失 ReAct Action 后执行一次受控 SQL 动作恢复",
            )
            yield sql_step_start
            yield step_meta(
                sql_step_id,
                "将唯一 SQL 映射交给原生只读 Tool 校验并执行",
                "sql_query",
                {"sql": sql_candidate},
                "sql_query",
            )
            recovered_chunks = emit_tool_chunks(sql_step_id, recovered_sql_text)
            if recovered_chunks:
                for chunk in recovered_chunks:
                    yield chunk
            else:
                yield step_chunk(sql_step_id, "text", recovered_sql_text)
            yield step_done(sql_step_id, recovered_status)
            history_steps.append(
                {
                    "id": sql_step_id,
                    "title": "sql_query",
                    "detail": "缺失 ReAct Action 后执行一次受控 SQL 动作恢复",
                    "thought": "将唯一 SQL 映射交给原生只读 Tool 校验并执行",
                    "action": "sql_query",
                    "action_input": json.dumps(
                        {"sql": sql_candidate}, ensure_ascii=False
                    ),
                    "outputs": [
                        {
                            "output_type": "text",
                            "content": recovered_sql_text,
                        }
                    ],
                    "status": recovered_status,
                }
            )
            react_state["sql_action_recovery_applied"] = True

    # Knowledge-only turns can exhibit the same missing Action envelope. Keep
    # the recovery shape as narrow as SQL recovery and execute the registered
    # semantic_search Tool so citations remain traceable to a real Observation.
    if is_knowledge_fast_path and not (
        isinstance(tool_pack, InsightReliabilityToolPack) and tool_pack.force_terminal
    ):
        raw_search_candidate = reply.content or ""
        if reply.action_report and reply.action_report.content:
            raw_search_candidate = reply.action_report.content
        search_candidate = _extract_search_candidate(raw_search_candidate)
        if search_candidate:
            knowledge_result = await tool_pack.async_execute(
                resource_name="semantic_search",
                query=search_candidate,
                top_k=5,
            )
            knowledge_text = (
                knowledge_result
                if isinstance(knowledge_result, str)
                else json.dumps(knowledge_result, ensure_ascii=False, default=str)
            )
            knowledge_step_id, knowledge_step_start = build_step(
                "semantic_search",
                "缺失 ReAct Action 后执行一次受控知识检索恢复",
            )
            yield knowledge_step_start
            yield step_meta(
                knowledge_step_id,
                "将唯一查询映射交给原生知识库 Tool",
                "semantic_search",
                {"query": search_candidate, "top_k": 5},
                "semantic_search",
            )
            recovered_chunks = emit_tool_chunks(knowledge_step_id, knowledge_text)
            if recovered_chunks:
                for chunk in recovered_chunks:
                    yield chunk
            else:
                yield step_chunk(knowledge_step_id, "text", knowledge_text)
            yield step_done(knowledge_step_id, "done")
            history_steps.append(
                {
                    "id": knowledge_step_id,
                    "title": "semantic_search",
                    "detail": "缺失 ReAct Action 后执行一次受控知识检索恢复",
                    "thought": "将唯一查询映射交给原生知识库 Tool",
                    "action": "semantic_search",
                    "action_input": json.dumps(
                        {"query": search_candidate, "top_k": 5},
                        ensure_ascii=False,
                    ),
                    "outputs": [{"output_type": "text", "content": knowledge_text}],
                    "status": "done",
                }
            )
            final_answer_assembler.observe(
            ...
            react_state["knowledge_action_recovery_text"] = knowledge_text

    # DeepSeek can state the correct intended Tool but omit the Action envelope.
    # Recover only when the required arguments are deterministically available.
    # The recovery is emitted as a real SSE step so evaluation and audit trails
    # see the native domain Tool instead of a fabricated final-answer claim.
    if (
        is_insight_integrated_mode
        and isinstance(tool_pack, SalesDiagnosisFirstToolPack)
        and tool_pack.last_domain_result is None
        and not tool_pack.force_terminal
    ):
        fallback_args = focused_domain_tool_args(
            user_input, tool_pack.required_domain_tool
        )
        if fallback_args is not None:
            try:
                fallback_result = await tool_pack.async_execute(
                    resource_name=tool_pack.required_domain_tool,
                    **fallback_args,
                )
                ...
                recovery_step_id, recovery_start = build_step(
                    tool_pack.required_domain_tool,
                    "缺失 Action 后执行一次受控原生领域 Tool 恢复",
                )
                yield recovery_start
                yield step_meta(
                    recovery_step_id,
                    "根据已识别业务意图补执行必需的领域 Tool",
                    tool_pack.required_domain_tool,
                    fallback_args,
                    tool_pack.required_domain_tool,
                )
                emitted = emit_tool_chunks(recovery_step_id, fallback_text)
                if emitted:
                    for chunk in emitted:
                        yield chunk
                else:
                    yield step_chunk(recovery_step_id, "text", fallback_text)
                yield step_done(recovery_step_id, "done")
                history_steps.append(
                    {
                        "id": recovery_step_id,
                        "title": tool_pack.required_domain_tool,
                        "detail": "缺失 Action 后执行一次受控原生领域 Tool 恢复",
                        "thought": "根据已识别业务意图补执行必需的领域 Tool",
                        "action": tool_pack.required_domain_tool,
                        "action_input": json.dumps(fallback_args, ensure_ascii=False),
                        "outputs": [{"output_type": "text", "content": fallback_text}],
                        "status": "done",
                    }
                )
                react_state["domain_recovery_applied"] = True
                logger.info(
                    "Applied domain Tool recovery tool=%s conv_id=%s",
                    tool_pack.required_domain_tool,
                    conv_id,
                )
            except Exception:
                logger.exception(
                    "Domain Tool recovery failed tool=%s conv_id=%s",
                    tool_pack.required_domain_tool,
                    conv_id,
                )

    # A full report also requires actual knowledge evidence. If the model
    # stopped after planning, perform one bounded retrieval with the original
    # question. Never synthesize policy text when retrieval did not occur.
    required_dealer_policy = "经销商管理制度" in user_input or (
        "经销商" in user_input and "制度" in user_input
    )
    has_required_knowledge = (
        tool_pack._knowledge_evidence_completed
        if isinstance(tool_pack, SalesDiagnosisFirstToolPack)
        else False
    )
    if required_dealer_policy and isinstance(tool_pack, SalesDiagnosisFirstToolPack):
        has_required_knowledge = "经销商分级与考核管理制度" in str(
            tool_pack.last_knowledge_result or ""
        )

    if (
        is_insight_integrated_mode
        and isinstance(tool_pack, SalesDiagnosisFirstToolPack)
        and tool_pack.require_knowledge_and_report
        and tool_pack._domain_tool_completed
        and not has_required_knowledge
        and "semantic_search" in tool_pack._resources
        and not tool_pack.force_terminal
    ):
        try:
            recovery_query = (
                "经销商分级与考核管理制度 第3.2节 连续下降预警 "
                "第4.2节 A级经销商连续下降处置"
                if required_dealer_policy
                else user_input
            )
            knowledge_result = await tool_pack.async_execute(
                resource_name="semantic_search",
            ...
            knowledge_step_id, knowledge_start = build_step(
                "semantic_search",
                "缺失制度证据后执行一次受控知识检索恢复",
            )
            ...
            yield step_meta(
                ...
                "使用用户原问题检索其指定制度与章节",
                "semantic_search",
                {"query": recovery_query, "top_k": 5},
                "semantic_search",
            )
            emitted = emit_tool_chunks(knowledge_step_id, knowledge_text)
            if emitted:
                for chunk in emitted:
                    yield chunk
            else:
            ...
            history_steps.append(
                {
                    "id": knowledge_step_id,
                    "title": "semantic_search",
                    "detail": "缺失制度证据后执行一次受控知识检索恢复",
                    "thought": "使用用户原问题检索其指定制度与章节",
                    "action": "semantic_search",
                    "action_input": json.dumps(
                        {"query": recovery_query, "top_k": 5},
                        ensure_ascii=False,
            ...
            react_state["knowledge_recovery_applied"] = True
            if required_dealer_policy:
                # A previously rendered report may contain the wrong policy.
                # Force the evidence-bound fallback to render it again.
                tool_pack._html_report_completed = False
            final_answer_assembler.observe(
                "semantic_search",
                {"query": recovery_query, "top_k": 5},
                knowledge_text,
            ...
            logger.info(
                "Applied knowledge recovery tool=semantic_search conv_id=%s",
                conv_id,
            )
        except Exception:
            logger.exception("Knowledge recovery failed conv_id=%s", conv_id)
    ...
    if is_knowledge_fast_path and react_state.get("knowledge_action_recovery_text"):
        final_content = react_state["knowledge_action_recovery_text"]

    clarification_questions = react_state.get("semantic_clarification_questions")
    if is_database_fast_path and clarification_questions:
        final_content = (
            "当前问题存在影响经营结论的高风险歧义，请先确认：\n- "
            + "\n- ".join(str(item) for item in clarification_questions)
        )
    elif is_database_fast_path and is_insight_database(database_name):
        verified_sql_answer = format_verified_sql_answer(
            user_input, react_state.get("last_sql_result")
        )
        if verified_sql_answer:
            final_content = verified_sql_answer

    # Focused InsightAgent questions use a deterministic presenter over the
    # verified native Tool Observation.  This also covers LLM turns that stop
    # after a Thought without emitting a valid terminate envelope.
    if (
        is_insight_integrated_mode
        and isinstance(tool_pack, SalesDiagnosisFirstToolPack)
        and not tool_pack.require_knowledge_and_report
    ):
        focused_answer = format_focused_domain_answer(
            tool_pack.required_domain_tool,
            tool_pack.last_domain_result,
            user_input,
        )
        if focused_answer:
            final_content = focused_answer

    # Render a recovery report only after both required evidence types exist.
    # The previous fallback could emit "completed" with neither a domain Tool
    # nor knowledge evidence, which made a plan-only response look successful.
    if (
        is_insight_integrated_mode
        and isinstance(tool_pack, SalesDiagnosisFirstToolPack)
        and tool_pack.require_knowledge_and_report
        and not tool_pack._html_report_completed
        and tool_pack._domain_tool_completed
        and tool_pack._knowledge_evidence_completed
        and not tool_pack.force_terminal
    ):
        domain_result = tool_pack.last_domain_result
        if isinstance(domain_result, str):
            try:
                domain_result = json.loads(domain_result)
            except Exception:
                domain_result = None
        domain_summary = format_focused_domain_answer(
            tool_pack.required_domain_tool,
            domain_result,
            user_input,
        )
        knowledge_text = str(tool_pack.last_knowledge_result or "")
        domain_json = html_lib.escape(
            json.dumps(domain_result, ensure_ascii=False, default=str)[:8000]
        )
        evidence_excerpt = html_lib.escape(knowledge_text[:6000])
        fallback_html = f"""<!DOCTYPE html><html lang='zh-CN'><head><meta charset='utf-8'>
<title>华东刹车系统销售经营分析</title><style>body{{font-family:Arial,sans-serif;padding:24px}}
table{{border-collapse:collapse;width:100%}}th,td{{border:1px solid #ddd;padding:8px;text-align:left}}
th{{background:#f3f6fa}}</style></head><body><h1>华东刹车系统销售经营分析</h1>
<h2>数据事实与规则结果</h2><pre>{domain_json}</pre>
<h2>制度检索证据</h2><pre>{evidence_excerpt}</pre>
<h2>管理建议</h2><p>本报告区分数据库事实、规则预警与管理建议。
应依据上方实际检索到的制度章节执行核验、整改计划和审批，
不以模型推断替代制度原文。</p></body></html>"""
        try:
            rendered = await tool_pack.async_execute(
                resource_name="html_interpreter",
                html=fallback_html,
                title="华东刹车系统销售经营分析",
            )
            rendered_text = (
            ...
            report_step_id, report_start = build_step(
                "html_interpreter",
                "依据真实领域与制度 Observation 渲染恢复报告",
            )
            yield report_start
            yield step_meta(
                report_step_id,
            ...
            rendered_chunks = emit_tool_chunks(report_step_id, rendered_text)
            if rendered_chunks:
                for chunk in rendered_chunks:
                    yield chunk
            else:
                yield step_chunk(report_step_id, "text", rendered_text)
            yield step_done(report_step_id, "done")
            history_steps.append(
                {
                    "id": report_step_id,
                    "title": "html_interpreter",
                    "detail": "依据真实领域与制度 Observation 渲染恢复报告",
                    "thought": "仅使用已取得的领域与制度证据生成报告",
                    "action": "html_interpreter",
                    "action_input": {"title": "华东刹车系统销售经营分析"},
                    "outputs": [{"output_type": "text", "content": rendered_text}],
                    "status": "done",
                }
            )
            final_content = (
                (domain_summary or "领域 Tool 已完成并返回可核验数据。")
                + "\n\n已依据真实领域 Tool Observation 和知识库检索证据"
                "完成 HTML 经营分析报告。"
            )
            if knowledge_text:
                final_content += "\n\n制度检索证据：\n" + knowledge_text[:4000]
        except Exception as exc:
            logger.warning("InsightAgent HTML fallback failed: %s", exc)

    if (
        is_insight_integrated_mode
        and isinstance(tool_pack, SalesDiagnosisFirstToolPack)
        and tool_pack.require_knowledge_and_report
        and tool_pack._missing_steps()
    ):
        final_content = (
            "综合经营分析未完成："
            + "；".join(tool_pack._missing_steps())
            + "。系统没有生成虚假的成功报告。"
        )
    elif (
        is_insight_integrated_mode
        ...
        integrated_answer = format_integrated_business_answer(
            tool_pack.required_domain_tool,
            tool_pack.last_domain_result,
            tool_pack.last_knowledge_result,
            user_input,
            tool_pack.domain_results,
        )
        if integrated_answer:
            final_content = integrated_answer

    if isinstance(tool_pack, InsightReliabilityToolPack):
        reliability_completion = tool_pack.completion_result()
        reliability_trace = {
            "type": "reliability.trace",
            "case_id": reliability_config.get("case_id"),
            "profile": reliability_config.get("profile"),
            "max_steps": max_retry_count,
            "completion": reliability_completion.to_dict(),
            "decisions": tool_pack.diagnostics,
        }
        if tool_pack.diagnostics:
            yield _sse_event(reliability_trace)
        if reliability_completion.status.value != "success" and (
            tool_pack.force_terminal or reliability_config.get("case_id")
        ):
            ...
            final_content = reliability_completion.to_user_text()
            if verified_partial:
                final_content = (
                    "以下数据结果已经完成并通过 Tool Observation 验证：\n"
                    + verified_partial
