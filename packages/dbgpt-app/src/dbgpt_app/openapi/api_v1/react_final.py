...
class AgentCitation:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.primary_section:
            result["primarySection"] = self.primary_section
        return result
...
class FinalAnswerAssembler:
    ...
    def finalize(self, content: Any) -> AgentFinalAnswer:
        ...
        citations = tuple(
            AgentCitation(
                ...
                path=draft.path,
                url=draft.url,
                primary_section=draft.primary_section,
            )
    ...
    def _append(self, draft: _CitationDraft) -> None:
        ...
        primary_section = _clean_primary_section(draft.primary_section)
...
def _adapt_explicit_citations(
    ...
    for item in citations:
        ...
        primary_section = item.get("primarySection") or item.get("primary_section")
        drafts.append(
            _CitationDraft(
                source_name=str(source_name),
                excerpt=excerpt,
                ...
                primary_section=str(primary_section) if primary_section else None,
            )
        )
    return drafts
...
def _clean_primary_section(value: Any) -> Optional[str]:
    """Keep only a numbered section identifier from trusted retrieval metadata."""

    if not isinstance(value, str):
        return None
    match = re.search(r"\b([1-9]\d*(?:\.\d+)+)\b", value)
