...
def _initialize_resource_manager(system_app: SystemApp):
    ...
    # InsightAgent domain tools use the shared FunctionTool contract. The ReAct
    # Web endpoint reads ResourceManager's ``tool`` instances into its
    # ToolPack, so this registration is the native discovery path rather than
    # a parallel, demo-only agent.
    rm.register_resource(resource_instance=sales_diagnosis_tool)
    rm.register_resource(resource_instance=customer_loss_analysis_tool)
    rm.register_resource(resource_instance=dealer_health_analysis_tool)
