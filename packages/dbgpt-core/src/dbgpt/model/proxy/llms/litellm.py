...
class LiteLLMClient(ProxyLLMClient):
    ...
    SDK), so a single InsightAgent model entry can talk to OpenAI, Anthropic, Vertex
    ...
    *without running a separate LiteLLM proxy server*. The model is selected
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # InsightAgent's generic per-request payload doesn't crash provider-specific
