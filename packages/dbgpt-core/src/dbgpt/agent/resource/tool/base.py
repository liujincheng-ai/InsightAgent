...
from dbgpt.util.configure.base import _MISSING
...
class ToolParameter(BaseModel):
    ...
    constraints: Dict[str, Any] = Field(
        ...
        description="JSON Schema constraints exposed to the model",
    )
        ...
        if prompt_type == "openai":
            ...
            for key, value in self.args.items():
                ...
                if value.constraints:
                    parameter["constraints"] = value.constraints
...
class FunctionTool(BaseTool):
    ...
    @property
    def validates_args(self) -> bool:
        """Return whether execution uses the opt-in Pydantic contract."""

        return self._validate_args and self._args_schema is not None
    ...
    def execute(
        ...
        **kwargs,
        ...
        if not self._validate_args or self._args_schema is None:
            return self._func(*args, **kwargs)
        validated, error = self._validated_call_args(args, kwargs)
    ...
    def _validated_call_args(
        self, args: tuple[Any, ...], kwargs: Dict[str, Any]
    ) -> tuple[Dict[str, Any], Optional[Dict[str, Any]]]:
        """Validate opt-in tool arguments before any side effect occurs."""

        positional = inspect.signature(self._func).bind_partial(*args)
        candidate = dict(positional.arguments)
        candidate.update(kwargs)
        try:
            model = self._args_schema.model_validate(candidate)
        except ValidationError as exc:
            return {}, _structured_validation_error(exc)
        validated = model_to_dict(model)
        function_args = set(inspect.signature(self._func).parameters)
        return {
            key: value for key, value in validated.items() if key in function_args
        }, None
...
def _parse_args_from_schema(args_schema: Type[BaseModel]) -> Dict[str, ToolParameter]:
    ...
    pydantic_args = schema["properties"]
    ...
    for key, value in pydantic_args.items():
        ...
        param_required = key in required_fields
        constraint_keys = {
            "enum",
            "pattern",
            "minimum",
        ...
        constraints = {name: value[name] for name in constraint_keys if name in value}
...
def _structured_validation_error(exc: ValidationError) -> Dict[str, Any]:
    """Return a stable, model-repairable error without executing the tool."""
