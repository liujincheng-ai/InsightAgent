...
class ToolPack(ResourcePack):
    ...
    def from_resource(
        ...
        if not resource:
            return []
        # Preserve a specialised ToolPack when it is already present in the
        # agent resource tree.  Rebuilding it as a plain ToolPack discards
        # intentional execution policy held by a native subclass.
        ...
        if isinstance(resource, ResourcePack):
            nested_packs = [
                ...
                if isinstance(sub_resource, ToolPack)
            ]
            if len(nested_packs) == 1:
                return [cast(T, nested_packs[0])]
    ...
    def _get_call_args(self, arguments: Dict[str, Any], tl: BaseTool) -> Dict[str, Any]:
        ...
        if isinstance(tl, FunctionTool) and tl.validates_args:
            # Preserve unknown keys for the opt-in Pydantic contract. Silently
            # deleting them would make ``extra='forbid'`` impossible to enforce.
            return arguments
