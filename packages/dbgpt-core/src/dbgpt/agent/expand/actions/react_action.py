...
def parse_strict_action_input(
    tool_name: str, action_input: Any
) -> Optional[Dict[str, Any]]:
    """Parse tools that require an exact JSON object contract.

    ``None`` means the tool keeps its legacy parsing behavior.  SQL deliberately
    fails closed so malformed model text is returned to the ReAct loop instead
    of being guessed into executable arguments.
    """
    ...
    if isinstance(action_input, str):
        try:
            ...
            raise ValueError(
                "SQL_ACTION_INPUT_INVALID_JSON: sql_query 的 Action Input 必须是"
                '完整 JSON，例如 {"sql": "SELECT ..."}'
            ) from error
    else:
        parsed = action_input
    if not isinstance(parsed, dict):
        raise ValueError("SQL_ACTION_INPUT_NOT_OBJECT: sql_query 参数必须是 JSON 对象")
    if set(parsed) != {"sql"}:
        raise ValueError(
            "SQL_ACTION_INPUT_SCHEMA_ERROR: sql_query 只允许一个必填字段 sql"
        )
    if not isinstance(parsed["sql"], str) or not parsed["sql"].strip():
        raise ValueError("SQL_ACTION_INPUT_SCHEMA_ERROR: sql 必须是非空字符串")
...
def normalize_common_tool_args(tool_name: str, tool_args: Any) -> Any:
    ...
    if tool_name == "kb_cat" and "path" not in normalized:
...
class ReActAction(ToolAction):
    ...
    async def _do_run(
        ...
        if name == "sql_query":
            try:
                tool_args = parse_strict_action_input(name, action_input) or {}
                action_input_str = json.dumps(tool_args, ensure_ascii=False)
                ...
                logger.warning("Rejected malformed sql_query Action Input: %s", error)
                return ActionOutput(
                    is_exe_success=False,
                    content=error_content,
                    observations=error_content,
        ...
        try:
            ...
            if name == "sql_query":
                pass
            elif action_input and isinstance(action_input, str):
        ...
        tool_args = normalize_common_tool_args(name, tool_args)
