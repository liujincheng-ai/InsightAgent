...
class ToolResultStorage:
    ...
    def begin_turn(self) -> None:
        """Reset the rolling inline-result budget for a new assistant turn."""
        self._turn_inline_chars = 0

    def maybe_persist_for_turn(
        self,
        content: str,
        tool_name: str,
        tool_call_id: str,
    ) -> Tuple[str, Optional[str]]:
        """Apply both the per-result and rolling per-turn result budgets."""
        threshold = self.resolve_threshold(tool_name)
        if (
            threshold != float("inf")
            and self._turn_inline_chars + len(content) > self._config.turn_budget
        ):
            threshold = 0
        replacement, path = self.maybe_persist(
            ...
            threshold=None if threshold == float("inf") else int(threshold),
        )
        self._turn_inline_chars += len(replacement)
    ...
    def read_persisted(self, file_path: str) -> Optional[str]:
        ...
        try:
            ...
            if os.path.commonpath([real_path, real_storage]) != real_storage:
                logger.warning(
