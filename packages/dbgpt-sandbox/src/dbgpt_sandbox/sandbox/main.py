"""
InsightAgent Sandbox Main Entry Point
"""
...
def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description="InsightAgent Sandbox Server")
...
def run_sandbox_server(
    ...
    logger.info(f"Starting InsightAgent Sandbox server on {host}:{port}")

    try:
        initialize_sandbox(host=host, port=port, log_level=log_level)
    except KeyboardInterrupt:
        logger.info("Shutting down InsightAgent Sandbox server...")
